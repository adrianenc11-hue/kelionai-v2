...
function calculateSuperConfluence(indicators) {
  ...
  const signals = {
    ...
    macd: indicators.macd?.crossSignal,
    bollinger: indicators.bollinger?.signal,
    ...
    parabolicSAR: indicators.parabolicSAR?.trend,
    ichimoku: indicators.ichimoku?.signal,
  ...
  for (const [key, sig] of Object.entries(signals)) {
    if (sig && scoreMap[sig] !== undefined) {
      weightedScore += scoreMap[sig] * (weights[key] || 5);
      totalWeight += weights[key] || 5;
    }
  }
  ...
  return {
    ...
    regime: indicators.marketRegime?.regime,
    details: signals,
  };
}
...
async function executeTrade(action, symbol, price, analysis, atrPct) {
  ...
  if (dailyTrades.length >= CONFIG.MAX_DAILY_TRADES)
    return {
      executed: false,
      reason: `Daily limit (${CONFIG.MAX_DAILY_TRADES})`,
    };
  if (openPositions.length >= CONFIG.MAX_OPEN_POSITIONS)
    return {
      executed: false,
      reason: `Max positions (${CONFIG.MAX_OPEN_POSITIONS})`,
    };
  if (now - lastLossTime < CONFIG.COOLDOWN_AFTER_LOSS_MS)
    return {
      executed: false,
      reason: `Cooldown: ${Math.round((CONFIG.COOLDOWN_AFTER_LOSS_MS - (now - lastLossTime)) / 1000)}s`,
    };
  ...
  // Weekly limit
  if (isWeeklyLimitHit(portfolioValue))
    return {
      executed: false,
      reason: `⛔ Weekly loss limit hit (${CONFIG.MAX_WEEKLY_LOSS_PCT * 100}%)`,
    };

  // Daily kill switch
  if (dailyPnL < -portfolioValue * CONFIG.MAX_DAILY_LOSS_PCT)
    return {
      executed: false,
      reason: `⛔ KILL SWITCH: Daily loss exceeds ${CONFIG.MAX_DAILY_LOSS_PCT * 100}%`,
    };
  ...
  if (size * price < 10)
    return {
      executed: false,
      reason: `Position too small: $${(size * price).toFixed(2)}`,
    };
...
module.exports = {
  ...
  getDailyTrades: () => [...dailyTrades],
  getPaperTrades: () => [...paperTrades],
  getPaperBalance: () => ({ ...paperBalance }),
  getDailyPnL: () => dailyPnL,
  getWeeklyPnL: () => weeklyPnL,
