...
async function scanAndHeal(brain, supabaseAdmin) {
  ...
  for (const [tool, count] of errored) {
    ...
    logger.warn({ component: "SelfHeal", tool, count, lastError: lastError.msg },
...
async function attemptRepair(brain, supabaseAdmin, tool, errorCount, lastError) {
  ...
  const toolFileMap = {
    search_web: "server/brain.js", search: "server/brain.js",
    get_weather: "server/brain.js", weather: "server/brain.js",
    generate_image: "server/brain.js", imagine: "server/brain.js",
    play_radio: "server/brain.js", play_video: "server/brain.js",
    get_news: "server/brain.js", open_website: "server/brain.js",
    check_system_health: "server/brain.js", recall_memory: "server/brain.js",
    memory: "server/brain.js",
    thinkV4: "server/brain-v4.js", thinkV5: "server/brain-v5.js",
  ...
  const codeSnippet = relevantLines.length > 0
    ? relevantLines.join("\n")
...
{"diagnosis": "...", "fix": null, "reason": "Why it cannot be auto-fixed", "confidence": 0}`;
  ...
  logger.info({ component: "SelfHeal", tool, diagnosis: analysis.diagnosis, confidence: analysis.confidence },
    `📋 Diagnosis: ${analysis.diagnosis}`);

  // Log diagnosis to brain_memory
  logToMemory(supabaseAdmin, tool, "diagnosed", JSON.stringify({
    diagnosis: analysis.diagnosis,
  ...
  if (!analysis.fix || analysis.confidence < 0.7) {
    logger.warn({ component: "SelfHeal", tool, confidence: analysis.confidence },
  ...
  try {
    ...
    logToMemory(supabaseAdmin, tool, "auto_fixed_deployed", JSON.stringify({
      diagnosis: analysis.diagnosis,
    ...
    fs.writeFileSync(fixFile, currentContent, "utf8");
    try { execSync("git checkout -- .", { cwd: PROJECT_ROOT, timeout: 5000 }); } catch (_) {}
