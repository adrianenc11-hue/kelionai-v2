...
async function preProcess(message, options = {}) {
    ...
    const context = {
        ...
        memories: memories.slice(0, 3).map(m => m.content).filter(Boolean),
        world: {
    ...
    logger.debug({
        platform, domain: reasoning.reasoning.domain,
...
function getK1SystemContext(context) {
    ...
    if (context.world?.btc) {
        parts.push(`[Market] BTC: $${context.world.btc}, ETH: $${context.world.eth || "?"}, Fear&Greed: ${context.world.fearGreed || "?"}`);
    }
