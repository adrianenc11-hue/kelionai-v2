...
function getModelStatus() {
  ...
  const overallBadge = models.some((m) => m.status === "update_available")
    ? "🟡"
    : models.every(
          (m) => m.status === "up_to_date" || m.status === "not_configured",
        )
      ? "🟢"
      : "🔴";
  ...
  return {
    ...
    overallBadge,
    overallStatus:
      overallBadge === "🟢" ? "All up to date" : "Updates available",
  ...
  try {
    ...
    if (key) {
      ...
      if (r.ok) {
        ...
        const latestPro =
          models.find((m) => m.includes("gemini-3.1-pro")) ||
          models.find((m) => m.includes("gemini-3.0-pro"));
        const latestFlash =
          models.find((m) => m.includes("gemini-3.1-flash")) ||
          models.find((m) => m.includes("gemini-3.0-flash"));
        ...
        logger.info(
          { availableModels: models.length, latestPro, latestFlash },
          "[ModelWatcher] Gemini models checked",
        );
      }
    }
  } catch (e) {
