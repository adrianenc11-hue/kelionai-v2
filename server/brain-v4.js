...
const TOOL_DEFINITIONS = [
  {
    ...
    description:
      'Display any HTML content (Leaflet map with markers, data table, chart, flight tracker, etc.) in the monitor panel. Use this to create rich visual displays. Generate the complete self-contained HTML with inline JS/CSS.',
    input_schema: {
      type: 'object',
      properties: {
        html: {
          type: 'string',
          description: 'Complete self-contained HTML to display in monitor (with inline styles and scripts)',
        },
  ...
  },
  {
    name: 'recall_tool',
    description:
      'Search the shared tool registry (Supabase) for a tool that can handle a specific task. Always try this FIRST before searching the web for a new API.',
    input_schema: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description: "Keywords describing the task (e.g. 'flight tracking', 'currency exchange', 'earthquake data')",
        },
      },
      required: ['query'],
    },
  },
  {
    ...
    description:
      'When no existing tool can handle a task: search the web for a free public API, test it, and save it permanently to the shared tool registry so all users benefit. Use this when recall_tool returns nothing.',
    input_schema: {
      type: 'object',
      properties: {
        task_description: {
          type: 'string',
          description: "What the tool needs to do (e.g. 'get real-time flight positions')",
        },
  ...
  },
  {
    name: 'show_map',
    description:
      'Show a location or navigation route on Google Maps. For navigation, provide origin and destination. For a simple location, provide just place.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'car_diagnostic',
    description:
      "Vehicle OBD-II diagnostic tool. Read trouble codes, check engine status, view live data from the car's computer.",
    input_schema: {
      type: 'object',
      properties: {
        action: {
          type: 'string',
          enum: ['read_codes', 'clear_codes', 'live_data', 'vehicle_info', 'emissions_check'],
  ...
  },
  {
    ...
    description:
      'Financial calculator: loan payments, interest, ROI, currency conversion, tax estimation, savings projections.',
    input_schema: {
      type: 'object',
      properties: {
        calculation: {
          type: 'string',
          enum: ['loan', 'interest', 'roi', 'currency', 'tax', 'savings', 'mortgage'],
  ...
  // ═══ GMAIL API (REAL EMAIL) ═════════════════════════════════
  {
    name: 'list_emails',
    description:
      "List recent emails from user's Gmail inbox. Shows sender, subject, date, and preview. Requires Gmail OAuth2 connection.",
    input_schema: {
      type: 'object',
      properties: {
        ...
        query: {
          type: 'string',
          description: "Gmail search query (e.g. 'from:boss@company.com', 'is:unread', 'subject:invoice')",
        },
      },
      required: [],
    },
  },
  {
    name: 'read_email',
    description:
      'Read the full content of a specific email by its ID. Returns full body, attachments info, and headers.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'draft_reply',
    description:
      'Create a draft reply to an email. The AI composes the reply based on context, but it is NOT sent automatically — saved as draft for user review.',
    input_schema: {
      type: 'object',
      properties: {
        ...
        tone: {
          type: 'string',
          enum: ['formal', 'friendly', 'professional', 'brief'],
  ...
  },
  {
    name: 'send_email',
    description:
      'Send an email via Gmail. Requires explicit user confirmation before sending. The email is first shown to user for approval.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  // ═══ SELF-INTROSPECTION TOOLS ═══════════════════════════════
  {
    ...
    description:
      'Read your own source code files. Use this when the user asks you to analyze, explain, or improve your own code. You can read server files, client JS, HTML, CSS. Returns file content (max 500 lines).',
    input_schema: {
      type: 'object',
      properties: {
        filepath: {
          type: 'string',
          description:
            "Relative path from project root, e.g. 'server/brain-v5.js', 'app/js/avatar.js', 'app/index.html', 'server/routes/translate.js'",
        },
  ...
  },
  {
    ...
    description:
      "Propose a code edit to your own source. The edit is NOT applied automatically — it's saved as a proposal for the admin to review and approve. Use this when the user asks you to fix a bug, add a feature, or improve your own code.",
    input_schema: {
      type: 'object',
      properties: {
  ...
  // ═══ HEADLESS BROWSER ═══════════════════════════════════════
  {
    name: 'browse_page',
    description:
      'Browse a real webpage with headless Puppeteer. Navigates to URL, can extract text content, take screenshots, or query CSS selectors. Use when you need to actually READ a web page, not just search.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  // ═══ CODE SANDBOX ═══════════════════════════════════════════
  {
    ...
    description:
      'Execute JavaScript code in a safe isolated sandbox. Use to test code, run calculations, validate logic, or demonstrate algorithms. Has access to Math, JSON, Date, Array, Object, String, Number, RegExp. No network, no filesystem, timeout 10s.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  // ═══ MULTI-TURN PLANNING ════════════════════════════════════
  {
    name: 'task_plan',
    description:
      'Manage persistent task plans that survive across messages. Use to break complex problems into steps, track progress, and maintain context across multi-turn conversations.',
    input_schema: {
      type: 'object',
      properties: {
        action: {
  ...
  // ═══ TERMINAL ACCESS ════════════════════════════════════════
  {
    name: 'run_terminal',
    description:
      'Execute a shell command on the server. Only whitelisted safe commands are allowed: git, npm, ls, cat, grep, find, wc, head, tail, echo, pwd, df, free, uptime. Use for checking git status, running tests, viewing files, searching code. Max 30s timeout.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  // ═══ MULTI-FILE AWARENESS ═══════════════════════════════════
  {
    name: 'list_directory',
    description:
      'List files and directories in your own project. Shows file names, sizes, and types. Use to understand project structure, find files, or see what files exist.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'Search through your own source code using grep. Find functions, variables, patterns, bugs, or any text across all project files. Returns matching lines with file paths and line numbers.',
    input_schema: {
      type: 'object',
      properties: {
...
const ADMIN_TOOL_DEFINITIONS = [
  ...
  },
  {
    name: 'admin_traffic',
    description:
      'Get detailed traffic data: recent visits, top pages, top countries, top referrers, hourly distribution. Admin only.',
    input_schema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: 'admin_users',
    ...
    switch (toolName) {
      ...
      case 'task_plan': {
        ...
        if (action === 'create') {
          ...
          const { data, error } = await brain.supabase
            .from('brain_memory')
            .insert({
              user_id: userId || null,
        ...
        if (action === 'get_active') {
          const { data } = await brain.supabase
            .from('brain_memory')
        ...
        if (action === 'list_all') {
          ...
          return {
            plans: (data || []).map((p) => ({
              ...
              steps_done: p.context?.steps?.filter((s) => s.done).length,
              steps_total: p.context?.steps?.length,
              created: p.created_at,
            })),
          };
        }
        ...
        if (action === 'update') {
          ...
          if (toolInput.steps)
            plan.steps = toolInput.steps.map((s, i) => ({ index: i, text: s, done: plan.steps?.[i]?.done || false }));
          await brain.supabase
            .from('brain_memory')
            .update({ context: plan, content: `[PLAN] ${plan.title}` })
            .eq('id', toolInput.plan_id);
      ...
      case 'search_own_code': {
        ...
        const searchPath = toolInput.path
          ? pathMod.resolve(projectRoot, (toolInput.path || '').replace(/\.\./g, ''))
          : projectRoot;
            ...
            try {
              ...
              const matches = lines
                ...
                .map((l) => {
                  const m = l.match(/^(\d+):(.*)/);
                  return m
                    ? { line: parseInt(m[1]), text: m[2].trim().substring(0, 150) }
                    : { text: l.substring(0, 150) };
                });
      ...
      case 'show_map': {
        ...
        return {
          monitorHTML: `<iframe src="${mapUrl}" style="width:100%;height:100%;border:0" allowfullscreen></iframe>`,
          type: 'html',
          success: true,
        };
      }
      ...
      case 'draft_reply': {
        ...
        if (!gmailRoute || !gmailRoute[toolName]) {
          return {
            error:
              'Gmail not configured. Admin must set up Google OAuth2 credentials (GMAIL_CLIENT_ID, GMAIL_CLIENT_SECRET) and the user must connect their Gmail account.',
          };
        }
      ...
      case 'admin_dashboard': {
        ...
        return {
          ...
          brain: {
            uptime: Math.round((Date.now() - brain.startTime) / 60000) + ' min',
            conversations: brain.conversationCount || 0,
          },
        };
      }
      case 'admin_traffic': {
        ...
        return {
          recentVisits: (recent || [])
            .slice(0, 10)
            .map((r) => ({ time: r.created_at, path: r.path, ip: r.ip, country: r.country || '?' })),
...
function extractMonitor(toolResults) {
  for (const r of toolResults) {
    if (r.result && typeof r.result === 'object') {
      if (r.result.mapHTML) return { content: r.result.mapHTML, type: 'html' }; // iframe embed direct
...
`;
    const systemPrompt =
      process.env.NEWBORN_MODE === 'true'
        ? buildNewbornPrompt(
            memoryBlock + patternsBlock + qualityHints + contextSwitchHint + proactiveHint + focusBlock
          )
        : buildSystemPrompt(
            avatar,
            language,
    ...
    const geminiTools = [{ functionDeclarations: toGeminiTools(allTools) }];
