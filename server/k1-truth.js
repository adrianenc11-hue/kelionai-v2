...
function extractClaims(text) {
  ...
  const priceRegex =
    /(BTC|ETH|SOL|Gold|NASDAQ|S&P|EUR\/USD|GBP\/USD)\s*(?:e|este|=|la|price|at)\s*\$?([\d,.]+)/gi;
  let match;
  ...
  const pctRegex =
    /(RSI|confidence|win rate|accuracy|return)\s*(?:e|este|=|de|la)?\s*([\d.]+)\s*%?/gi;
  ...
  const trendRegex =
    /(BTC|ETH|SOL|piața|market)\s*(crește|scade|bullish|bearish|urcă|coboară|stagnează)/gi;
...
function findContradictions(text) {
  ...
  if (
    /crește|bullish|urcă/i.test(text) &&
    /scade|bearish|coboară/i.test(text)
  ) {
    // Verificăm dacă sunt despre subiecte diferite
    const bullishMatch = text.match(
      /(BTC|ETH|SOL|Gold|piața|totul)\s*(crește|bullish|urcă)/i,
    );
    const bearishMatch = text.match(
      /(BTC|ETH|SOL|Gold|piața|totul)\s*(scade|bearish|coboară)/i,
    );
  ...
  if (
    /confidence\s*(8|9)\d/i.test(text) &&
    /nu sunt sigur|incert|posibil/i.test(text)
  ) {
    contradictions.push({
...
function verify(text, liveData = {}) {
  ...
  const verified = verifiedClaims.filter(
    (c) => c.verification.status === "verified",
  ).length;
  const falseClaims = verifiedClaims.filter(
    (c) => c.verification.status === "false",
  ).length;
  const unverifiable = verifiedClaims.filter(
  ...
  if (falseClaims > 0) {
    logger.warn(
      { falseClaims, trustScore },
      "[K1-Truth] ⚠️ Claims false detectate!",
    );
  }
...
function runSelfTest(domain = "trading") {
  ...
  const results = tests.map((test) => {
    ...
    const passed = Object.entries(test.expected).every(([key, val]) => {
      ...
      if (key === "hasPriceClaim")
        return report.claims.some((c) => c.type === "price") === val;
