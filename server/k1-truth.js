...
function extractClaims(text) {
    ...
    while ((match = trendRegex.exec(text)) !== null) {
        claims.push({
            type: "trend",
            subject: match[1],
            direction: /crește|bullish|urcă/i.test(match[2]) ? "up" : /scade|bearish|coboară/i.test(match[2]) ? "down" : "flat",
            text: match[0],
        });
    }
...
function checkPriceClaim(claim, liveData) {
    if (!liveData || !liveData[claim.asset]) {
        return { status: "unverifiable", reason: `Nu am date live pentru ${claim.asset}` };
    }
    ...
    if (diff <= tolerance) {
        return { status: "verified", emoji: "✅", claimed: claim.claimed, actual: real, diff: (diff * 100).toFixed(2) + "%" };
    } else if (diff <= 0.1) {
        return { status: "approximate", emoji: "⚠️", claimed: claim.claimed, actual: real, diff: (diff * 100).toFixed(2) + "%" };
    } else {
        return { status: "false", emoji: "❌", claimed: claim.claimed, actual: real, diff: (diff * 100).toFixed(2) + "%" };
    }
}
...
function verify(text, liveData = {}) {
    k1Cognitive.think(`Verificare text: "${text.slice(0, 80)}..."`, { phase: "OBSERVE" });
...
function generateSelfTest(domain = "trading") {
    const tests = {
        trading: [
            { input: "BTC e la $70000 și RSI e 25, deci e STRONG BUY", expected: { hasPriceClaim: true, hasTrend: false } },
            { input: "Piața crește dar și scade simultan", expected: { contradictions: 1 } },
            { input: "Confidence 95% dar nu sunt sigur", expected: { contradictions: 1 } },
        ],
        general: [
            { input: "Fără claims specifice, doar o observație generală", expected: { totalClaims: 0 } },
        ],
    };
...
function runSelfTest(domain = "trading") {
    ...
    const testRun = { domain, score, tests: results, timestamp: new Date().toISOString() };
    selfTestResults.push(testRun);
    if (selfTestResults.length > 20) selfTestResults.shift();

    k1Cognitive.think(`Self-test ${domain}: ${score}% (${results.filter(r => r.passed).length}/${results.length} passed)`, { phase: "LEARN", domain });
