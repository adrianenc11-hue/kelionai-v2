...
const ENV_GROUPS = {
  ...
  voice: [
    ...
    { key: 'DEEPGRAM_API_KEY', label: 'Deepgram STT (streaming)' },
    { key: 'CARTESIA_API_KEY', label: 'Cartesia TTS (streaming)' },
  ],
  payments: [
...
function validateEnv() {
  ...
  // In development, skip noisy env validation — just log a summary
  if (process.env.NODE_ENV !== 'production') {
    const allKeys = Object.values(ENV_GROUPS).flat();
    const present = allKeys.filter((v) => process.env[v.key]).length;
    logger.info(
      { component: 'EnvCheck', configured: present, total: allKeys.length },
      `🔑 Dev mode: ${present}/${allKeys.length} env vars configured`
    );
    return results;
  }
