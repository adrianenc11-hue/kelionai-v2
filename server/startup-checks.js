...
function validateEnv() {
    ...
    for (const [group, vars] of Object.entries(ENV_GROUPS)) {
        ...
        results.groups[group] = groupMissing.length === 0 ? "OK" : `MISSING: ${groupMissing.join(", ")}`;
    }

    // Critical check
    const criticalMissing = results.missing.filter(m => m.group === "critical");
    if (criticalMissing.length > 0) {
        logger.error({ component: "EnvCheck", missing: criticalMissing.map(m => m.key) },
    ...
    if (optionalMissing.length > 0) {
        logger.warn({ component: "EnvCheck", missing: optionalMissing.map(m => m.key) },
...
function setupGracefulShutdown(server) {
    ...
    const shutdown = async (signal) => {
        ...
        setTimeout(() => {
            logger.warn({ component: "Shutdown" }, "⚠️ Forced exit after 10s timeout");
            process.exit(1);
        }, 10000).unref();
    };
    ...
    logger.info({ component: "Shutdown" }, "🛡️ Graceful shutdown handlers registered");
}
...
async function smokeTest(port) {
    ...
    for (const route of routes) {
        try {
            ...
            if (!ok) {
                logger.warn({ component: "SmokeTest", path: route.path, status: r.status },
                    `⚠️ ${route.path} returned ${r.status} (expected ${route.expect})`);
            }
        } catch (e) {
            results.push({ path: route.path, status: "ERROR", ok: false, error: e.message });
            logger.warn({ component: "SmokeTest", path: route.path, err: e.message },
                `⚠️ ${route.path} failed: ${e.message}`);
        }
    }

    const passed = results.filter(r => r.ok).length;
    logger.info({ component: "SmokeTest", passed, total: results.length },
