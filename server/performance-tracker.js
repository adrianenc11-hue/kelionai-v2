...
class PerformanceTracker {
    ...
    _updateStats(trade) {
        ...
        if (!this.stats.bestTrade || pnl > (this.stats.bestTrade.pnl || 0)) {
            this.stats.bestTrade = { pnl: +pnl.toFixed(2), asset: trade.asset || trade.symbol, date: trade.created_at };
        }
        if (!this.stats.worstTrade || pnl < (this.stats.worstTrade.pnl || 0)) {
            this.stats.worstTrade = { pnl: +pnl.toFixed(2), asset: trade.asset || trade.symbol, date: trade.created_at };
        }
    }
     ...
     */
    getMaxDrawdown() {
        if (this.equityCurve.length < 2) return { pct: 0, amount: 0, peak: this.startingBalance, trough: this.startingBalance };
