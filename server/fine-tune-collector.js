...
function collectPair(systemPrompt, userMessage, assistantReply, metadata = {}) {
  try {
    ...
    logger.info({ component: 'FineTune', pairs: pairCount }, `📦 Fine-tune pair collected (#${pairCount})`);

    // Notify when ready for training
    if (pairCount > 0 && pairCount % 100 === 0) {
      logger.info(
        { component: 'FineTune', total: pairCount, ready: pairCount >= MIN_PAIRS_FOR_TRAINING },
...
function exportForTraining() {
  try {
    ...
    logger.info(
      { component: 'FineTune', file: cleanFile, pairs: lines.length },
...
try {
  ...
  if (fs.existsSync(JSONL_FILE)) {
    ...
    if (pairCount > 0) {
      logger.info({ component: 'FineTune', existing: pairCount }, `📦 Fine-tune: ${pairCount} existing pairs loaded`);
    }
  }
} catch (_) {
  /* ignored */
