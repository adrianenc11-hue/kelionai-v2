...
async function callClaudeCode(systemPrompt, message, history, options = {}) {
  ...
  if (!anthropicKey) {
    return { content: null, error: 'ANTHROPIC_API_KEY not configured', docsUrl: API_PROVIDERS.anthropic.docsUrl };
  }

  const { maxTokens = 16000, temperature = 0.2, model } = options;
  ...
  try {
    ...
    const resp = await fetch(`${API_ENDPOINTS.ANTHROPIC}/messages`, {
      method: 'POST',
      headers: {
    ...
    if (resp.ok) {
      ...
      logger.info(
        { component: 'BrainSelf.ClaudeCode', model: claudeModel, inputTokens: usage.input_tokens, outputTokens: usage.output_tokens },
        `✅ Claude Code: ${usage.input_tokens || 0}→${usage.output_tokens || 0} tokens`
      );
      return { content, provider: `claude-code-${claudeModel}`, usage };
    } else {
      const errBody = await resp.json().catch(() => ({}));
      logger.warn({ component: 'BrainSelf.ClaudeCode', status: resp.status, err: errBody }, 'Claude Code failed');
      return { content: null, error: `Claude API error: ${resp.status}` };
    }
  } catch (e) {
    logger.warn({ component: 'BrainSelf.ClaudeCode', err: e.message }, 'Claude Code error');
