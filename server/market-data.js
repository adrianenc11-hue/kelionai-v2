...
async function downloadAllHistory(supabase, days = 365) {
    ...
    for (const [asset, config] of Object.entries(ASSET_SOURCES)) {
        try {
            ...
            if (candles.length > 0) {
                ...
                results[asset] = { count: candles.length, latest: candles[candles.length - 1]?.close };
                logger.info({ asset, candles: candles.length, latest: candles[candles.length - 1]?.close },
    ...
    logger.info({ totalCandles, assets: Object.keys(results).length },
...
async function fetchCoinGeckoOHLC(coinId, days) {
    try {
        ...
        for (const [ts, open, high, low, close] of ohlcData) {
            const dateKey = new Date(ts).toISOString().slice(0, 10);
            if (!dailyMap[dateKey]) {
                dailyMap[dateKey] = { date: dateKey, open, high, low, close, volume: 0 };
            } else {
...
function startDownloader(supabase) {
    ...
    setTimeout(() => downloadAllHistory(supabase).catch(e => {
