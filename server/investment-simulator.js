      ...
      try {
        ...
        if (sim.returnPct > 0) {
          bestStrategies.push({
            ...
            winRate: sim.winRate,
            beatHold: sim.beatHold,
            annualized: sim.annualizedReturn,
          });
        }
      } catch (e) {
  ...
  for (const [asset, periods] of Object.entries(results)) {
    ...
    if (best && best[1].returnPct > 0) {
      rules.push({
        ...
        data: {
          period: best[0],
          returnPct: best[1].returnPct,
          winRate: best[1].winRate,
          maxDD: best[1].maxDrawdown,
        },
      });
    }
    ...
    if (highWin.length > 0) {
      ...
      rules.push({
        ...
        data: {
          period: bestWR[0],
          winRate: bestWR[1].winRate,
          trades: bestWR[1].totalTrades,
        },
      });
    }
