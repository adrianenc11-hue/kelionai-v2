...
async function saveAllAnalyses(assetsArray) {
  ...
  try {
    ...
    const { error } = await supabaseAdmin.from(TABLE.ANALYSES).insert(records);
...
async function saveSignal(signalData) {
  ...
  try {
    ...
    const { error } = await supabaseAdmin.from(TABLE.SIGNALS).insert(record);
...
async function saveTrade(tradeData) {
  ...
  try {
    ...
    const { error } = await supabaseAdmin.from(TABLE.TRADES).insert(record);
