...
async function cacheGet(key) {
  ...
  if (_redisClient && _redisAvailable) {
    try {
      ...
      logger.debug({ component: 'Cache', err: err.message }, 'Redis GET failed, falling back to memory');
      _redisAvailable = false;
    }
  }
  // Fallback in-memory
...
async function cacheSet(key, value, ttlSeconds = 300) {
  ...
  if (_redisClient && _redisAvailable) {
    try {
      ...
      logger.debug({ component: 'Cache', err: err.message }, 'Redis SET failed, falling back to memory');
      _redisAvailable = false;
    }
  }
  // Fallback in-memory — enforce max size to prevent OOM
  if (_memStore.size >= 10000) {
    const firstKey = _memStore.keys().next().value;
    _memStore.delete(firstKey);
  }
...
async function cacheDel(key) {
  if (_redisClient && _redisAvailable) {
    try {
      ...
      logger.debug({ component: 'Cache', err: err.message }, 'Redis DEL failed, falling back to memory');
      _redisAvailable = false;
    }
  }
  _memStore.delete(key);
