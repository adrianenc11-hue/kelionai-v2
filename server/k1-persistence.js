...
async function saveState(supabase) {
    ...
    try {
        ...
        if (performance) {
            entries.push({ key: "performance_report", value: performance.getReport() });
        }
    } catch { }
    ...
    try {
        ...
        if (metaLearning) {
            ...
            entries.push({ key: "evolution", value: metaLearning.getEvolutionReport() });
        }
    } catch { }
    ...
    try {
        ...
        if (cognitive) {
            entries.push({ key: "cognitive_meta", value: cognitive.getMetaCognition() });
        }
    } catch { }
...
async function loadState(supabase) {
    ...
    try { worldState = require("./k1-world-state"); } catch { }
    try { performance = require("./k1-performance"); } catch { }
    try { metaLearning = require("./k1-meta-learning"); } catch { }
    try { cognitive = require("./k1-cognitive"); } catch { }
