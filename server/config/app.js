...
const PLAN_CONFIG = {
  ...
  free: {
    ...
    features: (process.env.PLAN_FREE_FEATURES || 'Chat AI|Voice synthesis|3D Avatar|10 mesaje/zi').split('|').map(f => f.trim()).filter(Boolean),
    stripe_monthly_price_id: null,
    stripe_annual_price_id:  null,
  },
  ...
  pro: {
    ...
    features: (process.env.PLAN_PRO_FEATURES || 'Tot ce include Free|100 mesaje/zi|Web search|Image generation|Vision AI|Priority support').split('|').map(f => f.trim()).filter(Boolean),
  ...
  premium: {
    ...
    features: (process.env.PLAN_PREMIUM_FEATURES || 'Tot ce include Pro|Mesaje nelimitate|Claude & GPT-4o|Voice realtime|API access|White-label').split('|').map(f => f.trim()).filter(Boolean),
