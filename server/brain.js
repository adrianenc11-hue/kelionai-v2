...
//   CODE     → Groq (Llama 3.3 70B) → Gemini Flash — PRIMAR pe toți avatarii
...
${weatherContext}`;
    ...
    // ── CODE: Groq Llama 3.3 (PRIMAR pe toți avatarii) → Gemini → DeepSeek ──
    if (!reply && intent === INTENT.CODE) {
      const claudeCodePrompt = buildClaudeCodePrompt(avatar, language, memoryContext);
      const codeStart = Date.now();

      // Groq Code — agentul principal pentru cod pe Kelion și Kira
      ...
      if (claudeResult?.content) {
        ...
        trace.steps.push({ agent: 'GroqCode', status: 'ok', ms: Date.now() - codeStart });
        this._recordProvider('GroqCode', Date.now() - codeStart, false);
      } else {
    ...
    if (!reply) {
      ...
      const localResponses = {
        ro: {
          ...
          code: [`[EMOTION:thinking] Pentru asistență cu cod am nevoie de o cheie API (GROQ_API_KEY). Adaugă-le în .env și voi putea scrie cod complet pentru tine!`],
          default: [
        ...
        en: {
          ...
          code: [`[EMOTION:thinking] For code assistance I need an API key (GROQ_API_KEY). Add them to .env and I'll write complete code for you!`],
          default: [
...
Response: "${reply.substring(0, 1000)}"`;
    ...
    if (intent === INTENT.CODE) toolsUsed.push('groq_code');
...
Assistant: ${reply.substring(0, 500).replace(/</g, '&lt;').replace(/>/g, '&gt;')}`;
  ...
  getDiagnostics() {
    ...
    return {
      status: 'ok',
      version: 'brain-v3-groq-code-gps-weather',
