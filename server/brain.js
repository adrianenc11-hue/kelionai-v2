...
class KelionBrain {
  ...
  async callTool(toolId, params = {}, userId = null) {
    // ── SUPER-ADMIN CAPABILITIES (Interceptors & Gates) ──
    if (['ADMIN_DEPLOY', 'ADMIN', 'ADMIN_READ_FILE', 'ADMIN_LIST_DIR', 'ADMIN_WRITE_FILE'].includes(toolId)) {
      try {
        ...
        if (toolId === 'ADMIN_READ_FILE') {
          ...
          return { success: true, data: { content: content.substring(0, 30000) }, tool: toolId };
        }
        ...
        return { success: false, error: e.message, tool: toolId };
      }
    }
...
- ADMIN_DEPLOY: You have FULL ADMIN RIGHTS over your own codebase. The complete local path of the application is '${process.cwd()}'. Use this tool to autonomously run the deployment procedure ('npm run deploy:all') and push your code to production.
...
When asked "what can you do?" list these real capabilities. Use them proactively when relevant.`;
  ...
  async checkSemanticCache(query, userId) {
    ...
    try {
      ...
      if (bestMatch) {
        logger.info(
          { component: 'SemanticCache', similarity: bestScore.toFixed(3) },
  ...
  async loadMemory(userId, type, limit = 10, contextHint = '', intentTier = 'warm') {
    ...
    if (userHot && userHot.size > 0 && intentTier !== 'cold') {
      ...
      if (hotResults.length >= limit) {
        logger.info(
          { component: 'Memory', tier: 'hot', count: hotResults.length, userId: userId.substring(0, 8) },
  ...
  promoteToHot(userId, memoryItem) {
    ...
    logger.info(
      { component: 'Memory', tier: 'hot', userId: userId.substring(0, 8), memId },
...
8. La final: oferă un REZUMAT + 3 exerciții practice`,
  ...
  async _video(query) {
    ...
    if (ytMatch) {
      const ytEmbed = this.getToolUrl('youtube_embed') || 'https://www.youtube-nocookie.com/embed';
  ...
  async _tradeIntelligence() {
    try {
      ...
      const _stub = {
        ...
        getBestPairsNow: () => null,
        getStats: () => ({}),
      };
      const ti = (() => {
        try {
          return require('./trade-intelligence');
        } catch {
          return {
            fetchMarketNews: async () => [],
            calculateNewsSentiment: () => 0,
  ...
  async _memory(userId) {
    ...
    try {
      ...
      if (!error && data?.length) {
        return data
          .map((p) => `${p.key}: ${typeof p.value === 'object' ? JSON.stringify(p.value) : p.value}`)
          .join('; ');
      }
  ...
  async _recallTool(query) {
    ...
    if (!data?.length)
      return {
        found: false,
        message: `No tool found for: ${query}. Use discover_and_save_tool to find and save a new one.`,
      };
  ...
  async _discoverAndSaveTool({ task_description, api_endpoint, method = 'GET', params_schema = {}, tool_name }) {
    ...
    try {
      ...
      const { error } = await this.supabaseAdmin.from('brain_tools').upsert(
        {
          name: tool_name,
      ...
      return {
        ...
        message: `Tool '${tool_name}' saved to shared registry. Now use call_saved_tool to execute it.`,
      };
    } catch (e) {
  ...
  async _callSavedTool(toolName, params = {}) {
    ...
    const { data } = await this.supabaseAdmin.from('brain_tools').select('*').eq('name', toolName).single();
    ...
    try {
      ...
      this.supabaseAdmin
        .from('brain_tools')
        .update({ last_error: fetchErr.message })
        .eq('name', toolName)
        .then(() => {});
      return {
...
Raspunde STRICT JSON. Daca nimic: {}`;
  ...
  async _extractKnowledgeGraph(userId, userMessage, aiReply, existingFacts) {
    try {
      ...
      if (error && !error.message?.includes('does not exist')) {
        ...
        logger.info(
          { component: 'KnowledgeGraph', count: relationships.length },
  ...
  isToolDegraded(tool) {
    return (this.toolErrors[tool] || 0) > 0;
  }
  ...
  getDiagnostics() {
    ...
    const degraded = Object.entries(this.toolErrors)
      .filter(([t, c]) => c > 0 && !disabledTools.has(t))
      .map(([t]) => t);
    return {
