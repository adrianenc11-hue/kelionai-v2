...
const KELION_TOOLS = [
  ...
  },
  {
    ...
    properties: {
      ...
      model: { type: 'string', enum: ['google/gemma-4-31b-it:free', 'google/gemini-flash-1.5'], description: "Which model to use. Default is google/gemma-4-31b-it:free." },
    },
  ...
  try {
    ...
    async function fetchOpenRouter(reqBody) {
      ...
      if (!r.ok && r.status === 402 && !fallbackTriggered) {
        ...
        currentModel = 'google/gemma-4-31b-it:free';
        reqBody.model = currentModel;
