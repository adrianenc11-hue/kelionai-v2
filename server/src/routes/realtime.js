...
- show_on_monitor(kind, query, title?) — display something on the presentation monitor behind you in the scene. Use whenever the user asks to "show me", "open", "display", or "play" a map, weather, a page, a concept, or a live audio stream (in any language). Pick the right kind by the user's EXPLICIT INTENT WORD, not by association: "map" for any request that mentions map / hartă / mapa / mappa / carte / "unde sunt" / "show me my location" / "arată-mi locația" — STRICTLY a geographic map with a pin, NEVER a weather forecast even though weather also has a map; "weather" ONLY when the user explicitly says weather / vreme / meteo / temperatura / forecast / rain / wind / sun / clouds / snow; "video" for YouTube clips, "image" for photos, "wiki" for Wikipedia, "web" for arbitrary HTTPS URLs, "audio" for a directly-playable audio stream URL (e.g. live radio), or "clear" to blank the monitor. Call it again with a new query to swap the content. Shortcut: when the user asks for "Linux", "a Linux shell", "a terminal", "deschide Linux", "arată-mi un terminal", or similar — call show_on_monitor with kind="web" and query="https://webvm.io" (Debian running in the browser via WebAssembly; no install needed).
  ...
  },
  {
    ...
    properties: {
      kind: {
        ...
        description: "Type of content. STRICT MUTUAL EXCLUSION between 'map' and 'weather' — pick by the user's INTENT word, not by what is also visually present: 'map' = a geographic map of a place or the user's location (OpenStreetMap with a pin). Use this whenever the user says 'map', 'hartă', 'mappa', 'carte', 'mapa', 'show me my location', 'arată-mi locația', 'unde sunt' — even if no specific place is given. 'weather' = ONLY when the user explicitly asks about weather, temperature, rain, wind, forecast, sun, clouds, snow, or 'cum e vremea' / 'meteo' / 'forecast' / 'temperatura'. Never substitute 'weather' for a plain map request. 'video' = YouTube clip or search; 'image' = photo search; 'wiki' = Wikipedia article; 'web' = arbitrary URL (must start with https://); 'audio' = live audio stream URL (radio, podcast feed, .mp3/.aac/.m3u8) rendered as an HTML5 audio player on the monitor; 'clear' = blank the monitor.",
      },
