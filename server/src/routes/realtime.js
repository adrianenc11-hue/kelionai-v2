...
const KELION_TOOLS = [
  ...
  { name: 'multimedia_analyzer', description: "Super-module for analyzing multimedia files. Extracts context, transcript, and sentiment from video and audio files. Detects formats.", properties: { url: { type: 'string', description: "URL of the video or audio file." }, type: { type: 'string', description: "'video' or 'audio'." }, action: { type: 'string', description: "'analyze_content', 'extract_transcript', or 'detect_format'." } }, required: ['url', 'type'] },
  { name: 'document_parser', description: "Super-module for reading and parsing PDF, DOCX, and Spreadsheet (CSV) documents.", properties: { type: { type: 'string', description: "'pdf', 'docx', or 'spreadsheet'." }, url: { type: 'string', description: "Public HTTPS URL." }, base64: { type: 'string', description: "Base64 payload." }, file_id: { type: 'string', description: "Temporary ID of the uploaded file." }, data: { type: 'string', description: "Raw CSV data (only required if type is spreadsheet)." }, max_chars: { type: 'integer', description: "Cap on returned text length." } }, required: ['type'] },
  { name: 'ocr_engine', description: "Super-module for Optical Character Recognition (OCR). Extracts text from images or parses passports.", properties: { mode: { type: 'string', description: "'image' or 'passport'." }, url: { type: 'string', description: "Public HTTPS URL." }, base64: { type: 'string', description: "Base64 payload." }, file_id: { type: 'string', description: "Temporary ID of the uploaded file." }, lang: { type: 'string', description: "Tesseract language code (default 'eng')." }, max_chars: { type: 'integer', description: "Cap on returned text length." } }, required: ['mode'] },
  { name: 'image_generator_editor', description: "Super-module for generating, editing images, and creating QR codes.", properties: { action: { type: 'string', description: "'generate', 'edit', or 'qr_code'." }, prompt: { type: 'string', description: "Prompt for image generation (required if action=generate)." }, operation: { type: 'string', description: "'resize', 'grayscale', or 'rotate' (required if action=edit)." }, source: { type: 'string', description: "Path or URL to the source image (required if action=edit)." }, text: { type: 'string', description: "Text/URL to encode (required if action=qr_code)." }, width: { type: 'integer', description: "Target width." }, height: { type: 'integer', description: "Target height." }, angle: { type: 'integer', description: "Rotation angle." } }, required: ['action'] },
  { name: 'hardware_manager', description: "Super-module for controlling host hardware, peripherals, cameras, and system configurations.", properties: { action: { type: 'string', description: "'connect', 'disconnect', or 'configure'." }, device: { type: 'string', description: "Device name or type." } }, required: ['action', 'device'] },
  { name: 'cloud_manager', description: "Super-module for autonomous read/write to cloud storage (Google Drive, Dropbox, OneDrive).", properties: { action: { type: 'string', description: "'read', 'write', or 'list'." }, provider: { type: 'string', description: "'gdrive', 'dropbox', or 'onedrive'." }, path: { type: 'string', description: "Path to file or folder." } }, required: ['action', 'provider'] },
  { name: 'communication_hub', description: "Super-module for all communications (Email, SMS, Chat). Can draft or send messages.", properties: { action: { type: 'string', description: "'send_email', 'compose_draft', or 'send_sms'." }, to: { type: 'string', description: "Recipient contact details." }, subject: { type: 'string', description: "Subject line." }, body: { type: 'string', description: "Message body." } }, required: ['action', 'to', 'body'] },
  { name: 'automation_engine', description: "Super-module for webhooks and automations (Zapier, Make, IFTTT).", properties: { action: { type: 'string', description: "'zapier_trigger' or 'webhook_trigger'." }, webhook_url: { type: 'string', description: "Webhook URL to call." }, payload: { type: 'string', description: "JSON payload to send." } }, required: ['action', 'webhook_url'] },
  { name: 'devops_toolkit', description: "Super-module for DevOps and Git operations (GitHub PRs, Repo info, file reading).", properties: { action: { type: 'string', description: "'repo_info', 'list_files', 'read_file', 'create_pr', or 'manage_prs'." }, repo: { type: 'string', description: "Repository slug (e.g. owner/name)." }, branch: { type: 'string', description: "Branch name." }, path: { type: 'string', description: "Path to file (for read_file)." }, title: { type: 'string', description: "PR title." }, body: { type: 'string', description: "PR body/description." }, pr_action: { type: 'string', description: "'list', 'merge', or 'close' (for manage_prs)." }, issue_number: { type: 'integer', description: "PR number (for manage_prs)." } }, required: ['action'] },
  { name: 'scheduler_pro', description: "Super-module for time management, calendars, and task planning.", properties: { action: { type: 'string', description: "'read_calendar', 'create_ics', 'schedule_task', or 'plan_tasks'." }, query: { type: 'string', description: "Search query or task description." }, start: { type: 'string', description: "Start time/date." }, end: { type: 'string', description: "End time/date." }, attendees: { type: 'array', items: { type: 'object', properties: { email: { type: 'string' }, name: { type: 'string' } }, required: ['email'] } } }, required: ['action'] },
  { name: 'smart_monitor', description: "Super-module for setting up smart alerts and system monitors.", properties: { condition: { type: 'string', description: "Condition to trigger alert." }, action_to_take: { type: 'string', description: "What to do when triggered." } }, required: ['condition', 'action_to_take'] },
  { name: 'deep_memory_architect', description: "Super-module for long-term memory, session persistence, and history.", properties: { action: { type: 'string', description: "'context_cache', 'session_persist', 'remember_fact', 'learn_from_observation', or 'get_history'." }, key: { type: 'string', description: "Memory key." }, value: { type: 'string', description: "Memory value." } }, required: ['action'] },
  { name: 'task_orchestrator', description: "Super-module for parallel execution and complex plan routing.", properties: { action: { type: 'string', description: "'parallel' or 'execute_plan'." }, calls: { type: 'string', description: "JSON array of tools for parallel execution." }, plan: { type: 'string', description: "JSON array of steps for execute_plan." } }, required: ['action'] },
  { name: 'universal_executor', description: "Super-module for executing code, terminal commands, or regex.", properties: { action: { type: 'string', description: "'run_code', 'run_terminal', or 'run_regex'." }, language: { type: 'string', description: "Programming language." }, code: { type: 'string', description: "Code or command to execute." }, pattern: { type: 'string', description: "Regex pattern." }, input: { type: 'string', description: "Input text for regex." } }, required: ['action'] },
