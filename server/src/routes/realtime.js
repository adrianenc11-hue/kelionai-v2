...
const KELION_TOOLS = [
  ...
  { name: 'run_agent_eval', description: "List, check status, or run the 10 advanced agent capability evaluation tests (code generation, dependency install, bugfix, config, refactor, DB schema, web research, git workflow, PR payload, Playwright UI click). Use when the user asks 'rulează testele', 'evaluare agent', 'capability test', 'ce teste ai', 'apasă butonul test'.", properties: { action: { type: 'string', description: "'list' to show tests, 'status' to check availability, 'run' to execute." }, test_id: { type: 'string', description: "Test ID from 01 to 10, or 'all'. Optional for run." }, test_name: { type: 'string', description: "Partial test name for run. Optional." } }, required: ['action'] },
