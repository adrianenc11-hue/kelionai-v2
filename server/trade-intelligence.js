  ...
  if (priceLows.length >= 2) {
    ...
    if (b.val < a.val && b.oscVal > a.oscVal) {
      divergences.push({
        ...
        strength: 3,
        signal: 'BUY',
        description: 'Price lower low, oscillator higher low → reversal UP expected',
      });
    }
    if (b.val > a.val && b.oscVal < a.oscVal) {
      divergences.push({
        ...
        strength: 2,
        signal: 'BUY',
        description: 'Price higher low, oscillator lower low → trend continuation',
      });
    }
  }
  ...
  if (priceHighs.length >= 2) {
    ...
    if (b.val > a.val && b.oscVal < a.oscVal) {
      divergences.push({
        ...
        strength: 3,
        signal: 'SELL',
        description: 'Price higher high, oscillator lower high → reversal DOWN expected',
      });
    }
    if (b.val < a.val && b.oscVal > a.oscVal) {
      divergences.push({
        ...
        strength: 2,
        signal: 'SELL',
        description: 'Price lower high, oscillator higher high → trend continuation',
      });
    }
  }
  ...
  negators.forEach((neg) => {
    if (h.includes(neg)) {
      ...
      modBull.forEach((p) => {
        if (
          h.includes(neg + p) ||
          (h.indexOf(p) - h.indexOf(neg) < 15 && h.indexOf(p) > h.indexOf(neg) && h.indexOf(neg) >= 0)
        ) {
          score -= 20;
        }
      });
...
function getEconomicCalendarRisks() {
  ...
  if (day === 5 && date <= 7) {
    if (hour >= 12 && hour <= 15) {
      ...
      risks.push({
        ...
        risk: 'HIGH',
        action: 'NFP day — extreme volatility at 13:30 UTC. Avoid forex 1h before/after.',
      });
    }
  }
  ...
  if (date >= 10 && date <= 15 && day >= 2 && day <= 4) {
    if (hour >= 12 && hour <= 14) {
      risks.push({
        ...
        risk: 'HIGH',
        action: 'Potential CPI release at 13:30 UTC — high volatility for all markets.',
      });
    }
  }
  ...
  if (hour >= 13 && hour <= 16 && day >= 1 && day <= 5) {
    risks.push({
      ...
      risk: 'MEDIUM',
      action: 'Peak liquidity — best time for major pairs. Expect volatility spikes.',
    });
  }
  ...
  if (day === 5 && date >= 15 && date <= 21 && [2, 5, 8, 11].includes(month)) {
    risks.push({
      ...
      risk: 'HIGH',
      action: 'Options & futures expiry — extreme volume and volatility in stocks.',
    });
  }
...
function evaluateTradingRules(params) {
  ...
  rules.push({
    ...
    passed: !(
      adx?.adx > 30 &&
      ((action === 'BUY' && adx.signal === 'SELL') || (action === 'SELL' && adx.signal === 'BUY'))
    ),
    reason:
      adx?.adx > 30
