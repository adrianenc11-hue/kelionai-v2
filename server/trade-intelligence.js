...
function detectDivergence(prices, oscillatorValues, lookback = 30) {
  ...
  if (priceLows.length >= 2) {
    ...
    if (b.val < a.val && b.oscVal > a.oscVal) {
      divergences.push({
        type: "bullish",
        pattern: "Regular Bullish Divergence",
        strength: 3,
        signal: "BUY",
        description:
          "Price lower low, oscillator higher low → reversal UP expected",
      });
    }
    if (b.val > a.val && b.oscVal < a.oscVal) {
      divergences.push({
        ...
        signal: "BUY",
        description:
          "Price higher low, oscillator lower low → trend continuation",
      });
    }
  }
  ...
  if (priceHighs.length >= 2) {
    ...
    if (b.val > a.val && b.oscVal < a.oscVal) {
      divergences.push({
        type: "bearish",
        pattern: "Regular Bearish Divergence",
        strength: 3,
        signal: "SELL",
        description:
          "Price higher high, oscillator lower high → reversal DOWN expected",
      });
    }
    if (b.val < a.val && b.oscVal > a.oscVal) {
      divergences.push({
        ...
        signal: "SELL",
        description:
          "Price lower high, oscillator higher high → trend continuation",
      });
    }
  }
...
function calculatePivotPoints(high, low, close, _open) {
  ...
  let signal = "HOLD";
  if (lastPrice > classic.R1)
    signal = "BUY"; // Above R1 = bullish
  else if (lastPrice < classic.S1)
    signal = "SELL"; // Below S1 = bearish
  else if (lastPrice > classic.PP)
    signal = "BUY"; // Above pivot = mild bullish
...
function calculateAroon(highs, lows, period = 25) {
  if (highs.length < period + 1)
    return { aroonUp: 50, aroonDown: 50, oscillator: 0, signal: "HOLD" };
  ...
  return {
    aroonUp: Math.round(aroonUp * 100) / 100,
    aroonDown: Math.round(aroonDown * 100) / 100,
...
function detectAdvancedChartPatterns(prices) {
  ...
  if (
    ...
    const handleDip =
      (Math.max(...handleSlice) - Math.min(...handleSlice)) /
      Math.max(...handleSlice);
    if (handleDip > 0.01 && handleDip < 0.1) {
      patterns.push({
        pattern: "Cup & Handle",
        type: "bullish",
        strength: 3,
        depth: Math.round(cupDepth * 100) + "%",
      });
    }
  }
  ...
  for (let i = 2; i < recent30.length - 2; i++) {
    if (recent30[i] > recent30[i - 1] && recent30[i] > recent30[i + 1])
      highs30.push({ i, v: recent30[i] });
    if (recent30[i] < recent30[i - 1] && recent30[i] < recent30[i + 1])
      lows30.push({ i, v: recent30[i] });
  }
  if (highs30.length >= 2 && lows30.length >= 2) {
    const hSlope =
      (highs30[highs30.length - 1].v - highs30[0].v) /
      (highs30[highs30.length - 1].i - highs30[0].i);
    const lSlope =
      (lows30[lows30.length - 1].v - lows30[0].v) /
      (lows30[lows30.length - 1].i - lows30[0].i);
  ...
  if (n > 30) {
    ...
    const flagRange =
      (Math.max(...flag) - Math.min(...flag)) / Math.max(...flag);
...
async function fetchMarketNews(asset = "crypto") {
  ...
  try {
    ...
    const r = await fetch(
      `https://cryptopanic.com/api/free/v1/posts/?auth_token=free&public=true&filter=hot&currencies=${asset}`,
    );
    if (r.ok) {
  ...
  return {
    headlines: [],
    overallSentiment: { score: 0, label: "neutral", signal: "HOLD" },
    source: "unavailable",
  };
}
...
function classifyNewsSentiment(headline) {
  ...
  strongBull.forEach((p) => {
    if (h.includes(p)) {
      score += 30;
      matchCount++;
    }
  });
  strongBear.forEach((p) => {
    if (h.includes(p)) {
      score -= 30;
      matchCount++;
    }
  ...
  negators.forEach((neg) => {
    if (h.includes(neg)) {
      ...
      modBull.forEach((p) => {
        if (
          h.includes(neg + p) ||
          (h.indexOf(p) - h.indexOf(neg) < 15 &&
            h.indexOf(p) > h.indexOf(neg) &&
            h.indexOf(neg) >= 0)
        ) {
          score -= 20;
        }
...
function calculateNewsSentiment(headlines) {
  if (!headlines || headlines.length === 0)
    return { score: 0, label: "neutral", signal: "HOLD" };
  const totalScore =
    headlines.reduce((sum, h) => sum + h.sentiment.score, 0) / headlines.length;
  const label =
    totalScore > 15 ? "bullish" : totalScore < -15 ? "bearish" : "neutral";
  const signal =
    label === "bullish" ? "BUY" : label === "bearish" ? "SELL" : "HOLD";
  return {
...
function getEconomicCalendarRisks() {
  ...
  if (fomcWindow) {
    risks.push({
      event: "FOMC Decision Day",
      risk: "CRITICAL",
      action:
        "⛔ Federal Reserve rate decision TODAY — extreme volatility expected. DO NOT trade forex 2h before/after announcement (typically 18:00 UTC).",
    });
  }
  ...
  if (day === 5 && date <= 7) {
    if (hour >= 12 && hour <= 15) {
      risks.push({
        event: "Non-Farm Payrolls LIVE",
        risk: "CRITICAL",
        action: "⛔ NFP release at 13:30 UTC — PAUSE all forex trading NOW.",
      });
    } else {
      risks.push({
        event: "Non-Farm Payrolls Day",
        risk: "HIGH",
        action:
          "NFP day — extreme volatility at 13:30 UTC. Avoid forex 1h before/after.",
      });
    }
  }
  ...
  if (date >= 10 && date <= 15 && day >= 2 && day <= 4) {
    if (hour >= 12 && hour <= 14) {
      risks.push({
        ...
        risk: "HIGH",
        action:
          "Potential CPI release at 13:30 UTC — high volatility for all markets.",
      });
    }
  }
  ...
  if (day === 0 || day === 6) {
    risks.push({
      event: "Weekend",
      risk: "HIGH",
      action: "Markets closed — only crypto tradeable",
      tradeable: { crypto: true, forex: false, stocks: false },
    });
  }
  ...
  if (hour >= 0 && hour < 8) {
    risks.push({
      event: "Asian Session (Tokyo)",
      risk: "LOW",
      action: "Low liquidity for Western pairs. Best for: JPY, AUD, NZD pairs.",
    });
  }
  if (hour >= 7 && hour < 16) {
    risks.push({
      event: "European Session (London)",
      risk: "LOW",
      action: "High liquidity for EUR, GBP. London open at 07:00 UTC.",
    });
  }
  if (hour >= 13 && hour < 21) {
    risks.push({
      event: "US Session (New York)",
      risk: "LOW",
      action: "Highest volume period. NY open at 13:30 UTC.",
    });
  }
  ...
  if (hour >= 13 && hour <= 16 && day >= 1 && day <= 5) {
    risks.push({
      event: "London/NY Overlap",
      risk: "MEDIUM",
      action:
        "Peak liquidity — best time for major pairs. Expect volatility spikes.",
    });
  }
  ...
  if (hour >= 13 && hour <= 15 && day >= 1 && day <= 5) {
    risks.push({
      event: "US Market Open Window",
      risk: "MEDIUM",
      action: "Volatility spike at 14:30 UTC — wider stops recommended.",
    });
  }
  ...
  if (date >= 28) {
    risks.push({
      ...
      risk: "MEDIUM",
      action: "Institutional rebalancing flows — unexpected moves possible.",
    });
  }
  ...
  if (day === 5 && date >= 15 && date <= 21 && [2, 5, 8, 11].includes(month)) {
    risks.push({
      event: "Quad Witching Day",
      risk: "HIGH",
      action:
        "Options & futures expiry — extreme volume and volatility in stocks.",
    });
  }
...
function evaluateTradingRules(params) {
  ...
  rules.push({
    ...
    passed: !(
      adx?.adx > 30 &&
      ((action === "BUY" && adx.signal === "SELL") ||
        (action === "SELL" && adx.signal === "BUY"))
    ),
    reason:
      adx?.adx > 30
        ? `ADX=${adx.adx} strong trend detected — only trade WITH the trend`
        : "No strong trend — both directions allowed",
    priority: "CRITICAL",
  });
  ...
  rules.push({
    ...
    reason: atrPct
      ? `ATR/Price = ${(atrPct * 100).toFixed(1)}% — ${atrPct >= 0.08 ? "TOO VOLATILE" : "acceptable"}`
      : "No ATR data",
    priority: "HIGH",
  });
  ...
  rules.push({
    ...
    reason: economicRisks?.shouldPause
      ? "High-impact economic event imminent — WAIT"
      : "No imminent events",
    priority: "HIGH",
  });
  ...
  rules.push({
    ...
    passed: !(
      (action === "BUY" && rsi?.value > 75) ||
      (action === "SELL" && rsi?.value < 25)
    ),
    reason: rsi
      ? `RSI=${rsi.value} — ${action === "BUY" && rsi.value > 75 ? "OVERBOUGHT, dont buy" : action === "SELL" && rsi.value < 25 ? "OVERSOLD, dont sell" : "OK"}`
      : "No RSI",
    priority: "MEDIUM",
  });
  ...
  rules.push({
    ...
    passed:
      !(fearGreed?.value > 80 && action === "BUY") &&
      !(fearGreed?.value < 20 && action === "SELL"),
    reason: fearGreed
      ? `F&G=${fearGreed.value} — ${fearGreed.value > 80 && action === "BUY" ? "EXTREME GREED + BUY = DANGEROUS" : fearGreed.value < 20 && action === "SELL" ? "EXTREME FEAR + SELL = DANGEROUS" : "OK"}`
      : "No F&G data",
    priority: "HIGH",
  });

  const _allPassed = rules.every((r) => r.passed || r.priority === "MEDIUM");
  const criticalFailed = rules.filter(
    (r) => !r.passed && r.priority === "CRITICAL",
  );
