...
function setupDebate(task, _options = {}) {
  ...
  k1Cognitive.think(`Debate setup: PRO=#${proAgent.id}, CONTRA=#${conAgent.id}, Judge=#${judgeAgent.id}`, {
    phase: 'PLAN',
  });
...
function setupEnsemble(task, agentTypes = ['research', 'trading', 'critic']) {
  ...
  k1Cognitive.think(`Ensemble setup: ${ensembleAgents.map((a) => `#${a.id}(${a.type})`).join(', ')}`, {
    phase: 'PLAN',
  });
...
async function executeDebate(task, options = {}) {
  ...
  k1Cognitive.think(`Debate finalizat în ${elapsed}ms — Verdict: ${(verdict || '').slice(0, 100)}`, {
    phase: 'OBSERVE',
  });
