        ...
        switch (event.type) {
          ...
          case 'response.audio_transcript.done': {
            if (event.transcript) {
              ...
              // Accumulate AI transcript for Supabase
          ...
          case 'conversation.item.input_audio_transcription.completed': {
            if (event.transcript) {
              ...
              // Accumulate user transcript for Supabase
              pendingUserTranscript += (pendingUserTranscript ? ' ' : '') + event.transcript;
              ...
              (async () => {
                try {
                  if (!voiceConvId) {
                    ...
                    else logger.warn({ component: 'VoiceRealtime', err: error?.message }, 'Conv insert failed');
                  } else {
                    await supabaseAdmin
                      .from('conversations')
                  ...
                  if (voiceConvId) {
                    ...
                    if (rows.length) {
                      ...
                      if (msgErr) logger.warn({ component: 'VoiceRealtime', err: msgErr.message }, 'Msg insert failed');
                      else
                        logger.info(
                          { component: 'VoiceRealtime', convId: voiceConvId, msgs: rows.length },
