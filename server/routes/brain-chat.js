...
function loadFileContent(filename) {
  ...
  } catch (e) { return `(Eroare citire ${filename}: ${e.message})`; }
}
...
function processToolCall(toolCall) {
  ...
  switch (tool) {
    case "readFile": {
      ...
      try {
        ...
        if (params.startLine && params.endLine) {
          const s = Math.max(1, params.startLine) - 1, e = Math.min(lines.length, params.endLine);
          return { result: lines.slice(s, e).map((l, i) => `${s + i + 1}: ${l}`).join("\n") };
        }
        return { result: `${filePath} (${lines.length} linii, ${stat.size}B):\n${content.slice(0, 50000)}` };
      } catch (e) { return { result: `Eroare citire: ${e.message}` }; }
    }
    case "searchCode": return { result: JSON.stringify(kiraTools.projectSearch(params.query, params.path)) };
    case "listFiles": return { result: JSON.stringify(kiraTools.projectTree(params.dir, params.depth)) };
    case "gitStatus": return { result: JSON.stringify(kiraTools.gitStatus()) };
    case "gitLog": return { result: JSON.stringify(kiraTools.gitLog(params.n)) };
    case "gitDiff": return { result: JSON.stringify(kiraTools.gitDiff()) };
    case "runTests": return { result: JSON.stringify(kiraTools.runTests(params.suite)) };
    case "queryDB": return { needsAsync: true, tool: "queryDB", params };
    case "screenshot": return { needsAsync: true, tool: "screenshot", params };
    case "browse": return { needsAsync: true, tool: "browse", params };
    case "webSearch": return { needsAsync: true, tool: "webSearch", params };
    case "readUrl": return { needsAsync: true, tool: "readUrl", params };
    ...
    case "editFile": {
      ...
      let cur = ""; try { cur = fs.readFileSync(path.resolve(fp), "utf8"); } catch { }
      let occ = 0;
      let editFn;
      if (startLine && endLine) {
        ...
        editFn = () => {
          if (target && occ > 0) {
            ...
            fs.writeFileSync(path.resolve(fp), newLines.join('\n'), 'utf8');
            occ = 1;
          } else { return { success: false, error: 'Text negăsit în range' }; }
          return { success: true, file: fp };
        };
      } else {
        occ = target ? (cur.split(target).length - 1) : 0;
        editFn = () => { if (!target || occ === 0) return { success: false, error: 'Text negăsit' }; fs.writeFileSync(path.resolve(fp), cur.replace(target, replacement), 'utf8'); return { success: true, file: fp }; };
      }
      _pendingOps.set(opId, {
        type: 'editFile', preview: { file: fp, find: (target || '').slice(0, 500), replace: (replacement || '').slice(0, 500), occ, startLine, endLine },
        execute: editFn
      });
      return { pendingApproval: true, opId, message: `⚠️ APROBARE\nEdit: ${fp}${startLine ? ` (L${startLine}-${endLine})` : ''}\nFind: ${(target || '').slice(0, 200)}\nReplace: ${(replacement || '').slice(0, 200)}\n(${occ} potriviri)`, preview: _pendingOps.get(opId).preview };
    }
    case "writeFile": {
      const opId = `OP_${++_opCounter}`;
      const fp = params.filePath || params.path, content = params.content;
      let cur = ""; try { cur = fs.readFileSync(path.resolve(fp), "utf8"); } catch { }
      _pendingOps.set(opId, {
        type: "writeFile", preview: { file: fp, curLen: cur.length, newLen: (content || "").length },
        execute: () => { const d = path.dirname(path.resolve(fp)); if (!fs.existsSync(d)) fs.mkdirSync(d, { recursive: true }); fs.writeFileSync(path.resolve(fp), content, "utf8"); return { success: true, file: fp, size: content.length }; }
      });
      return { pendingApproval: true, opId, message: `⚠️ APROBARE\nScrie: ${fp} (${(content || "").length} chars)`, preview: _pendingOps.get(opId).preview };
    }
    case "runCommand": {
      const opId = `OP_${++_opCounter}`;
      const cmd = params.command || params.cmd;
      _pendingOps.set(opId, {
        type: "runCommand", preview: { command: cmd },
        execute: () => { try { const out = execSync(cmd, { timeout: 30000, encoding: "utf8", maxBuffer: 1024 * 1024, cwd: process.cwd() }); return { success: true, output: (out || "").slice(0, 20000) }; } catch (e) { return { success: false, error: e.message, output: ((e.stdout || "") + (e.stderr || "")).slice(0, 10000) }; } }
      });
      return { pendingApproval: true, opId, message: `⚠️ APROBARE\nComandă: ${cmd}` };
    }
    case "deploy": {
      const opId = `OP_${++_opCounter}`;
      _pendingOps.set(opId, {
        type: "deploy", preview: { action: "railway up" },
        execute: () => { try { const out = execSync("npx -y @railway/cli up --detach 2>&1", { timeout: 120000, encoding: "utf8", maxBuffer: 2 * 1024 * 1024, cwd: process.cwd() }); return { success: true, output: (out || "").slice(0, 10000) }; } catch (e) { return { success: false, error: e.message }; } }
      });
      return { pendingApproval: true, opId, message: "⚠️ APROBARE\nDeploy pe Railway?" };
    }
    case "browseWithAuth": {
      const opId = `OP_${++_opCounter}`;
      _pendingOps.set(opId, {
        type: "browseWithAuth", params,
        preview: { url: params.url, actions: params.actions },
        execute: async () => {
          let puppeteer; try { puppeteer = require("puppeteer"); } catch { return { success: false, error: "Puppeteer nu e instalat" }; }
          let browser;
          try {
            browser = await puppeteer.launch({ headless: "new", args: ["--no-sandbox", "--disable-setuid-sandbox", "--disable-dev-shm-usage"], timeout: 30000 });
            ...
            if (params.actions && Array.isArray(params.actions)) {
              for (const a of params.actions) {
                try {
                  if (a.type === "fill") { await page.waitForSelector(a.selector, { timeout: 5000 }); await page.type(a.selector, a.value, { delay: 50 }); results.push({ action: "fill", ok: true }); }
                  else if (a.type === "click") { await page.waitForSelector(a.selector, { timeout: 5000 }); await page.click(a.selector); await page.waitForTimeout(2000); results.push({ action: "click", ok: true }); }
                  else if (a.type === "wait") { await page.waitForTimeout(a.ms || 2000); results.push({ action: "wait", ok: true }); }
                  else if (a.type === "select") { await page.select(a.selector, a.value); results.push({ action: "select", ok: true }); }
                } catch (e) { results.push({ action: a.type, ok: false, error: e.message }); }
              }
            }
            const content = await page.evaluate(() => ({ title: document.title, url: window.location.href, text: document.body?.innerText?.slice(0, 15000) || "" }));
            await browser.close();
      ...
      return { pendingApproval: true, opId, message: `⚠️ APROBARE\nBrowser: ${params.url}\nAcțiuni: ${JSON.stringify(params.actions || []).slice(0, 300)}`, preview: _pendingOps.get(opId).preview };
    }
    case "mutateDB": {
      const opId = `OP_${++_opCounter}`;
      _pendingOps.set(opId, { type: "mutateDB", params, preview: { table: params.table, op: params.operation, data: params.data }, execute: null });
    ...
    default: return { result: `Tool necunoscut: ${tool}` };
  }
}
...
async function callK1(systemPrompt, userMessage) {
  for (const p of [
    ...
    try { const r = await p.fn(); if (r) return { text: r, provider: p.name }; } catch (e) {
...
async function callGemini(system, message) {
  const key = process.env.GOOGLE_AI_KEY || process.env.GEMINI_API_KEY; if (!key) return null;
  const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${key}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ systemInstruction: { parts: [{ text: system }] }, contents: [{ parts: [{ text: message }] }] })
  });
...
// ═══ ROUTES ═══
let _session = null;
function getSession(supabase) { if (!_session) _session = new BrainSession(supabase); return _session; }
...
router.post("/", async (req, res) => {
  try {
    ...
    const BANNED = ['spune-mi ce vrei','ce aspect dore','pot ajuta','te pot ajuta','nu e practic','nu pot afisa','nu am capacitat','cum doresti','te rog sa specifici','sunt aici pentru','nu este practic','ce anume vrei','cum vrei sa procedam'];
    ...
    if (BANNED.some(p => low.includes(p)) && !response.text.includes('```json')) {
      ...
      const force = systemPrompt + '\n\nRASPUNSUL ANTERIOR A FOST BLOCAT. EXECUTA DIRECT: "' + message + '". Foloseste TOOL-URI (readFile, searchCode, listFiles). Raspunde cu FAPTE si COD, nu cu vorbe.';
      ...
      try {
        ...
        if (tr.needsAsync) {
          if (tr.tool === "queryDB" && supabase) {
            const { data, error } = await supabase.from(tr.params.table).select(tr.params.select || "*").limit(tr.params.limit || 20);
            tr = { result: JSON.stringify({ data, error: error?.message }, null, 2) };
          } else if (tr.tool === "screenshot") {
            tr = { result: JSON.stringify(await kiraTools.renderPage(tr.params.url, { screenshot: true })) };
          } else if (tr.tool === "browse") {
            tr = { result: JSON.stringify(await kiraTools.renderPage(tr.params.url, { screenshot: false })) };
          } else if (tr.tool === "webSearch") {
            const sk = process.env.SERPER_API_KEY;
            if (sk) {
              const sr = await fetch("https://google.serper.dev/search", { method: "POST", headers: { "Content-Type": "application/json", "X-API-KEY": sk }, body: JSON.stringify({ q: tr.params.query, num: 10 }) });
              tr = { result: JSON.stringify(await sr.json(), null, 2).slice(0, 10000) };
            } else { tr = { result: "SERPER_API_KEY lipsește" }; }
          } else if (tr.tool === "readUrl") {
            tr = { result: JSON.stringify(await kiraTools.scrapeUrl(tr.params.url), null, 2).slice(0, 15000) };
            ...
            if (imgKey) {
              const imgRes = await fetch("https://api.openai.com/v1/images/generations", {
                ...
                body: JSON.stringify({ model: "dall-e-3", prompt: tr.params.prompt, n: 1, size: tr.params.size || "1024x1024" })
              });
              const imgData = await imgRes.json();
              tr = { result: JSON.stringify({ url: imgData.data?.[0]?.url, revised_prompt: imgData.data?.[0]?.revised_prompt }) };
            } else { tr = { result: "OPENAI_API_KEY lipsește pentru imagini" }; }
          }
        }
        toolResults.push(tr);
      } catch (e) { toolResults.push({ result: `Eroare tool: ${e.message}` }); }
    }
    // Combină rezultatele
    if (toolResults.length === 1) toolResult = toolResults[0];
    else if (toolResults.length > 1) toolResult = { result: toolResults.map((tr, i) => `[Tool ${i+1}] ${tr.result || tr.message || 'OK'}`).join('\n\n') };
    ...
    res.json({ reply: brainMessage, provider: response.provider, sessionId: sid, toolResult: toolResult || null, pendingApproval: toolResult?.pendingApproval ? { opId: toolResult.opId, preview: toolResult.preview } : null });
  } catch (e) {
...
router.get("/history", async (req, res) => {
  try { const s = getSession(req.app?.locals?.supabase); res.json({ sessions: await s.listSessions() }); }
  catch (e) { res.status(500).json({ error: e.message }); }
});

// GET /api/admin/brain-chat/session/:id
router.get("/session/:id", async (req, res) => {
  try { const s = getSession(req.app?.locals?.supabase); res.json(await s.getSession(req.params.id)); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.post("/approve", async (req, res) => {
  try {
    const { opId } = req.body; if (!opId) return res.status(400).json({ error: "lipsește opId" });
    const op = _pendingOps.get(opId); if (!op) return res.status(404).json({ error: `Op ${opId} nu există` });
    let result = op.execute ? (typeof op.execute === "function" ? await op.execute() : op.execute) : { error: "No executor" };
