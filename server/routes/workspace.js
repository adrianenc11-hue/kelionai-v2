...
Răspunde în română, structurat și clar.`;
  ...
  // Groq este acum primar (Llama 3.3 70B)
  ...
  if (!analysisText && process.env.GOOGLE_AI_KEY) {
    try {
      const geminiModel = MODELS.GEMINI_CHAT || 'gemini-2.0-flash';
      const resp = await fetch(
        `${API_ENDPOINTS.GEMINI}/models/${geminiModel}:generateContent?key=${process.env.GOOGLE_AI_KEY}`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ contents: [{ role: 'user', parts: [{ text: prompt }] }], generationConfig: { maxOutputTokens: 1500 } }),
        }
      );
      const data = await resp.json();
