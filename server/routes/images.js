...
router.post('/', imageLimiter, validate(imagineSchema), async (req, res) => {
  try {
    ...
    if (!process.env.TOGETHER_API_KEY) return res.status(400).json({ error: 'Image generation not configured' });
    ...
    incrementUsage(user?.id, 'image', supabaseAdmin).catch((e) =>
      logger.warn({ component: 'Images', err: e.message }, 'incrementUsage failed')
    );
    // ═══ BRAIN INTEGRATION — save what we generated ═══
    if (brain && user?.id) {
      brain
        .saveMemory(user.id, 'visual', 'Am generat imagine: ' + prompt.substring(0, 400), { engine: 'FLUX' })
        .catch(() => {});
    }
