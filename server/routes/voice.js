  ...
  try {
    ...
    if (user && supabaseAdmin) {
      try {
        ...
        logger.warn({ component: 'Voice', err: e.message }, 'no cloned voice, use default');
      }
    }
      ...
      try {
        ...
        const gVoices = {
          ro: {
            ...
            kira: { name: 'ro-RO-Chirp3-HD-Laomedeia', lang: 'ro-RO' },
          },
    ...
    if (!buf && process.env.ELEVENLABS_API_KEY) {
      try {
        ...
        logger.warn({ component: 'Speak', err: e.message }, 'ElevenLabs TTS error (timeout?) — trying OpenAI fallback');
      }
    }
    ...
    incrementUsage(user?.id, 'tts', supabaseAdmin).catch((e) =>
      logger.warn({ component: 'Voice', err: e.message }, 'incrementUsage failed')
    );
