...
router.post('/voice/clone', upload.single('audio'), async (req, res) => {
  try {
    ...
    res.json({
      ...
      voiceId,
      name: voiceName,
      message: 'Voice cloned successfully! All TTS responses will now use your voice.',
    });
    ...
    if (brain && user?.id) {
      brain
        .saveFact(user.id, 'User a clonat vocea: ' + voiceName + ' (ID: ' + voiceId + ')', 'voice', 'voice-clone')
        .catch(() => {});
    }
  } catch (e) {
...
router.delete('/voice/clone', async (req, res) => {
  try {
    ...
    if (brain && user?.id) {
      brain
        .saveMemory(user.id, 'context', 'User a șters vocea clonată (ID: ' + voiceId + ')', { type: 'voice-clone' })
        .catch(() => {});
    }
  } catch (e) {
