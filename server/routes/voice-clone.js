...
router.post("/voice/clone", upload.single("audio"), async (req, res) => {
  try {
    ...
    if (!user)
      return res.status(401).json({ error: "Login required to clone voice" });
    ...
    if (req.file.size < 50000) {
      return res.status(400).json({
        error:
          "Audio too short. Please record at least 30 seconds of clear speech.",
      });
    }
    ...
    if (!cloneResp.ok) {
      ...
      logger.error(
        { component: "VoiceClone", status: cloneResp.status, error: errData },
        "ElevenLabs clone failed",
      );
      return res.status(500).json({
        error:
          "Voice cloning failed: " +
          (errData.detail?.message || errData.detail || "Unknown error"),
      });
    }
    ...
    logger.info(
      { component: "VoiceClone", userId: user.id, voiceId },
  ...
  try {
    ...
    if (process.env.ELEVENLABS_API_KEY) {
      try {
        ...
        logger.warn(
          { component: "VoiceClone", err: e.message },
          "ElevenLabs delete failed",
        );
      }
    }
