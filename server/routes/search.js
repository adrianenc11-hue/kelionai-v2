  ...
  try {
      ...
      try {
        ...
        if (r.ok) {
          ...
          incrementUsage(user?.id, 'search', supabaseAdmin).catch((e) =>
            logger.warn({ component: 'Search', err: e.message }, 'incrementUsage failed')
          );
          if (brain && user?.id)
            brain
              .saveMemory(user.id, 'search', 'User a căutat: ' + query + ' → ' + answer.substring(0, 300), {
                engine: 'Perplexity',
              })
              .catch(() => {});
        ...
        logger.warn({ component: 'Search', engine: 'Perplexity', err: e.message }, 'Perplexity');
      }
    }
    ...
    if (process.env.TAVILY_API_KEY) {
      try {
        ...
        if (tr.ok) {
          ...
          if (brain && user?.id)
            brain
              .saveMemory(user.id, 'search', 'User a căutat: ' + query + ' → ' + tavilyAnswer.substring(0, 300), {
                engine: 'Tavily',
              })
              .catch(() => {});
          return res.json(
        ...
        logger.warn({ component: 'Search', engine: 'Tavily', err: e.message }, 'Tavily');
      }
    }
    ...
    if (process.env.SERPER_API_KEY) {
      try {
        ...
        logger.warn({ component: 'Search', engine: 'Serper', err: e.message }, 'Serper');
      }
    }
    ...
    if (brain && user?.id)
      brain
        .saveMemory(user.id, 'search', 'User a căutat: ' + query + ' → ' + (d.Abstract || '').substring(0, 300), {
          engine: 'DuckDuckGo',
        })
        .catch(() => {});
...
router.get('/', searchLimiter, async (req, res) => {
  ...
  try {
    ...
    const usage = await checkUsage(user?.id, 'search', supabaseAdmin);
    if (!usage.allowed) return res.status(429).json({ error: 'Search limit reached.', plan: usage.plan });
