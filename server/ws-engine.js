...
class WSEngine extends EventEmitter {
    ...
    stop() {
        if (this.connections.binance) { this.connections.binance.close(); this.connections.binance = null; }
        if (this.connections.oanda) { this.connections.oanda.destroy?.(); this.connections.oanda = null; }
    ...
    _handleBinanceKline(msg) {
        ...
        if (candle.closed) {
            ...
            this.emit("candle", { asset: symbol, tf: "1m", candle, source: "binance" });
    ...
    _startOanda() {
        const host = this.config.oandaEnv === "live"
    ...
    _handleOandaTick(data) {
        ...
        this.emit("tick", { asset: pair, price: mid, bid, ask, spread, ts, source: "oanda" });
    }
    ...
    _startStockPolling() {
        const nameMap = { "^GSPC": "SP500", "^IXIC": "NASDAQ", "GC=F": "Gold", "CL=F": "Oil" };
                ...
                try {
                    ...
                    if (r.ok) {
                        ...
                        if (closes.length > 0) {
                            ...
                            this.emit("tick", { asset: name, price, ts: Date.now(), source: "yahoo" });
                        }
                    }
    ...
    async _flushToDb() {
        ...
        try {
            ...
            const { error } = await this.supabase
                .from("market_candles")
                .upsert(rows, { onConflict: "asset,timeframe,open_time", ignoreDuplicates: true });
    ...
    _reconnect(exchange) {
        ...
        logger.info({ component: "WSEngine", exchange, delay, attempt: attempts }, `Reconnecting ${exchange} in ${delay}ms...`);
        setTimeout(() => {
