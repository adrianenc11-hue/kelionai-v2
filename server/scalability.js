...
function getCircuitStats() {
    ...
    for (const [name, c] of Object.entries(_circuits)) {
        stats[name] = { state: c.state, failures: c.failures, successes: c.successes };
    }
    return stats;
}
...
setInterval(() => {
    ...
    for (const [ip, exp] of _banExpiry.entries()) {
        if (now > exp) { _blacklist.delete(ip); _banExpiry.delete(ip); }
    }
    // Reset counters
...
function getBlacklistStats() {
    return { banned: _blacklist.size, tracked: _ipCounts.size, list: Array.from(_blacklist) };
}
...
function gracefulDegradationMiddleware(req, res, next) {
    _requestsInFlight++;
    res.on("finish", () => { _requestsInFlight--; });
...
function getQueueStats() {
    return { pending: _taskQueue.length, running: _taskRunning, maxParallel: MAX_PARALLEL_TASKS };
}
