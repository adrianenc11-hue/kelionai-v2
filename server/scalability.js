...
function ipBlacklistMiddleware(req, res, next) {
  ...
  if (_blacklist.has(ip)) {
    return res.status(429).json({ error: 'Too many requests. Try again later.', retryAfter: 600 });
  }
  // Count
  const now = Date.now();
  ...
  if (data.count > IP_MAX) {
    ...
    return res.status(429).json({ error: 'Rate limit exceeded. Banned for 10 minutes.', retryAfter: 600 });
  }
  next();
}
...
function staticCacheMiddleware(req, res, next) {
  ...
  if (/\.(js|css)(\?|$)/i.test(url)) {
    // JS/CSS: NO cache — Cloudflare was caching old versions for 24h
    res.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate');
    res.setHeader('Pragma', 'no-cache');
    res.setHeader('Expires', '0');
    res.setHeader('CDN-Cache-Control', 'no-store');
    res.setHeader('Cloudflare-CDN-Cache-Control', 'no-store');
  } else if (/\.(woff2?|ttf|eot|glb|gltf)(\?|$)/i.test(url)) {
    // Fonts/3D models: cache 1 day (rarely change)
    res.setHeader('Cache-Control', 'public, max-age=86400');
  } else if (/\.(svg|png|jpg|jpeg|webp|gif|ico|mp3|wav)(\?|$)/i.test(url)) {
    // Images/audio: cache 1 hour
