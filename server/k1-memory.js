...
async function saveToWarm(supabase, entry) {
  ...
  try {
    ...
    logger.debug(
      { id: data?.id, type: record.type },
      "[K1-Memory] Salvat în warm memory",
    );
    return data;
  } catch (e) {
...
async function retrieve(supabase, query, options = {}) {
  ...
  const allResults = [...hotResults, ...warmResults].map((m) => {
    const age =
      (now - new Date(m.timestamp || m.created_at).getTime()) /
      (1000 * 60 * 60); // ore
    ...
    const matchedWords = queryWords.filter(
      (w) => w.length > 2 && contentLower.includes(w),
    );
    const semanticScore =
      queryWords.length > 0 ? matchedWords.length / queryWords.length : 0;

    const relevance =
      semanticScore * 0.4 + recencyDecay * 0.3 + importanceWeight * 0.3;

    return {
      ...m,
      relevance: Math.round(relevance * 100) / 100,
      source: m.id > 1e12 ? "hot" : "warm",
    };
  });
...
async function forget(supabase, options = {}) {
  ...
  logger.info(
    { deleted, compressed, archived },
    "[K1-Memory] 🧹 Forgetting cycle complete",
  );
...
async function archiveToCold(supabase, options = {}) {
  ...
  try {
    ...
    for (const [domain, memories] of Object.entries(byDomain)) {
      ...
      const contentParts = memories.map(
        (m) => `[${m.type}] ${(m.content || "").slice(0, 150)}`,
      );
      const avgImportance = Math.round(
        memories.reduce((s, m) => s + (m.importance || 5), 0) / memories.length,
      );
      ...
      await supabase.from("k1_memory").insert({
        content:
          `[COLD ARCHIVE | ${domain} | ${dateRange} | ${memories.length} items]\n${contentParts.join("\n")}`.slice(
            0,
            2000,
          ),
        type: "cold_archive",
...
async function createMemoryTable(supabase) {
  try {
    ...
    logger.warn(
      { err: e.message },
      "[K1-Memory] Nu am putut crea tabelul (creează-l manual)",
    );
  }
}
...
/**
 * NEWBORN RESET — Golește hot memory complet
 */
function clearHot() {
  hotMemory.length = 0;
  logger.info("[K1-Memory] 🧒 Hot memory cleared — newborn mode");
}
