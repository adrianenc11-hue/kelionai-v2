...
function searchHot(query) {
  ...
  return hotMemory.filter((m) => m.content.toLowerCase().includes(lower)).sort((a, b) => b.importance - a.importance);
}
...
async function saveToWarm(supabase, entry) {
  ...
  try {
    ...
    const { data, error } = await supabase.from('k1_memory').insert(record).select().single();
...
async function archiveToCold(supabase, options = {}) {
  ...
  try {
    ...
    for (const [domain, memories] of Object.entries(byDomain)) {
      ...
      const contentParts = memories.map((m) => `[${m.type}] ${(m.content || '').slice(0, 150)}`);
