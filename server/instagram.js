...
router.post("/webhook", async (req, res) => {
  ...
  try {
    ...
    logger.error(
      { component: "Instagram", err: e.message },
...
async function handleIncomingDM(msg) {
  ...
  logger.info(
    { component: "Instagram", senderId, text: text.substring(0, 50) },
    "DM received",
  );
...
async function sendDM(recipientId, text) {
  ...
  try {
    ...
    if (data.error) {
      logger.error(
        { component: "Instagram", error: data.error },
        "Send DM failed",
      );
    }
  } catch (e) {
...
async function createMediaContainer(imageUrl, caption) {
  if (!PAGE_TOKEN || !IG_ACCOUNT_ID) {
    logger.warn(
      { component: "Instagram" },
      "Missing INSTAGRAM_ACCOUNT_ID or FB_PAGE_ACCESS_TOKEN",
    );
    return null;
  }
  try {
    ...
    if (data.id) {
      logger.info(
        { component: "Instagram", containerId: data.id },
...
async function publishMedia(containerId) {
  ...
  try {
    ...
    if (data.id) {
      logger.info(
        { component: "Instagram", mediaId: data.id },
