...
const strategies = {
    ...
    promptTemplates: {
        trading_analysis: {
            ...
            uses: 0, successes: 0, lastUsed: null,
        },
        research_query: {
    ...
    toolPreferences: {
        price_source: { preferred: "yahoo", alternatives: ["coingecko", "binance"], scores: { yahoo: 80, coingecko: 70, binance: 75 } },
        news_source: { preferred: "gnews", alternatives: ["newsapi", "guardian"], scores: { gnews: 75, newsapi: 60, guardian: 80 } },
    },
...
function useTemplate(name, wasSuccessful) {
    ...
    if (t.uses >= 10 && t.successes / t.uses < 0.5) {
        k1Cognitive.think(`Template "${name}" are accuracy ${Math.round(t.successes / t.uses * 100)}% — necesită rescriere`, { phase: "LEARN" });
    }
}
...
function scoreTools(toolName, category, score) {
    ...
    if (best && best[0] !== pref.preferred) {
        ...
        k1Cognitive.think(`Tool switch: ${category} ${old} → ${best[0]} (scor ${best[1]} vs ${pref.scores[old]})`, { phase: "LEARN" });
    }
}
...
function adjustRisk(tradeResult) {
    ...
    if (reason && reason.includes("STOP-LOSS")) {
        ...
        if (sl.current < sl.max) {
            ...
            k1Cognitive.think(`Stop-loss ajustat: ${sl.current - sl.adjustStep}% → ${sl.current}% (prea multe stop-loss-uri)`, { phase: "LEARN" });
        }
    }
    ...
    if (reason && reason.includes("TRAILING-STOP") && pnlPct > 0 && pnlPct < 3) {
        ...
        if (tp.current > tp.min + tp.adjustStep) {
            ...
            k1Cognitive.think(`Take-profit ajustat: ${tp.current + tp.adjustStep}% → ${tp.current}% (trailing stop la profit mic)`, { phase: "LEARN" });
        }
    }
    ...
    if (reason && reason.includes("TIME-LIMIT") && pnlPct > 0) {
        ...
        if (mh.current < mh.max) {
            ...
            k1Cognitive.think(`Hold time ajustat: ${mh.current - mh.adjustStep}h → ${mh.current}h (closing profitable trades too early)`, { phase: "LEARN" });
        }
    }
}
...
function recordUserInteraction(data) {
    ...
    userModel.correctionRate = userModel.totalInteractions > 0
        ? Math.round(userModel.totalCorrections / userModel.totalInteractions * 100)
        : 0;
...
function startScheduledJobs(supabaseGetter) {
    ...
    forgettingInterval = setInterval(async () => {
        try {
            const sb = typeof supabaseGetter === "function" ? supabaseGetter() : supabaseGetter;
            if (!sb) return;
            const k1Memory = require("./k1-memory");
            const result = await k1Memory.forget(sb, { maxAge: 90, minImportance: 3 });
            k1Cognitive.think(`🧹 Forgetting: ${result.deleted} șterse, ${result.compressed} comprimate`, { phase: "LEARN" });
    ...
    selfTestInterval = setInterval(() => {
        try {
            ...
            k1Cognitive.think(`🧪 Self-test trading: ${result.score}% (${result.tests.filter(t => t.passed).length}/${result.tests.length})`, { phase: "LEARN" });
