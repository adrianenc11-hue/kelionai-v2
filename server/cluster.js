...
const MAX_WORKERS = parseInt(process.env.CLUSTER_WORKERS || "0", 10) || Math.min(os.cpus().length, 4);
...
if (cluster.isPrimary || cluster.isMaster) {
    ...
    cluster.on("exit", (worker, code, signal) => {
        console.log(`[Cluster] ⚠️ Worker ${worker.process.pid} died (${signal || code}). Restarting in ${RESTART_DELAY}ms...`);
