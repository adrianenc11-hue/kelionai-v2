...
class AIScoring {
  ...
  async scoreSignal(signalData) {
    ...
    try {
      ...
      if (!response.ok) {
        logger.warn(
          { component: "AIScoring", status: response.status },
          "AI API failed, using fallback",
        );
        return this._fallbackScore(signalData);
      }
      ...
      this.stats.avgScore = Math.round(
        (this.stats.avgScore * (this.stats.totalScored - 1) + result.score) /
          this.stats.totalScored,
      );
      ...
      logger.error(
        { component: "AIScoring", err: e.message },
        "AI scoring failed",
      );
      return this._fallbackScore(signalData);
    }
...
{"score": <0-100>, "action": "<STRONG_BUY|BUY|HOLD|SELL|STRONG_SELL>", "risk": "<low|medium|high|extreme>", "reason": "<30 words max>"}`;
  ...
  _fallbackScore(data) {
    ...
    if (data.smartMoney?.signal === conf?.signal?.replace("STRONG ", ""))
      score += 10;
