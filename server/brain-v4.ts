  ...
  try {
    ...
    if (ENV.openaiApiKey) {
      const res = await fetch("https://api.openai.com/v1/chat/completions", {
        ...
        body: JSON.stringify({ model: "gpt-4.1", messages: [{ role: "user", content: [{ type: "text", text: prompt }, { type: "image_url", image_url: { url: imageUrl, detail: "high" } }] }], max_tokens: 1000 }),
      });
      if (res.ok) {
...
export async function processBrainMessage(params: {
  ...
  // TTS is now on-demand only — generated by client request via voice.generateSpeech
  // No automatic TTS generation to save ElevenLabs credits

  return { content: finalContent, toolsUsed, confidence, userLevel, language };
}
