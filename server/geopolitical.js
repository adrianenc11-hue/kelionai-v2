      ...
      if (res.ok) {
        ...
        if (obs.length > 0) {
          result.indicators[id] = {
            ...
            trend:
              obs.length > 1 ? (parseFloat(obs[0].value) > parseFloat(obs[1].value) ? 'rising' : 'falling') : 'unknown',
          };
        }
      }
    } catch {
...
async function calculateGeopoliticalRisk() {
  ...
  const [gdelt, fearGreed, fred] = await Promise.all([fetchGdeltEvents(), fetchFearGreed(), fetchFredData()]);
...
function getHistoricalRiskContext(currentCategory) {
  ...
  const avgSeverity = similar.length > 0 ? Math.round(similar.reduce((s, e) => s + e.severity, 0) / similar.length) : 0;
