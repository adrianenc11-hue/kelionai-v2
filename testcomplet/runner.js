  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (e.name === 'AbortError')
        return { status: 408, data: 'Timeout', ok: false, elapsed: timeout, headers: {}, ct: '' };
      if (attempt < 3) {
        await sleep(3000);
        continue;
      }
...
const FUNCS = [
  // AUTH 1-9
  {
    ...
    body: { email: 'test_' + Date.now() + '@test.com', password: 'TestP@ss123!' },
  ...
  // (Legacy tests removed: trading 39-57, messenger 58-75, whatsapp 76-88, telegram 89-97)
  // INSTAGRAM 98-105
  {
    id: 98,
    name: 'IG Verify',
...
const runners = {
  async Unit(f) {
    if (f.type === 'api') {
      ...
      return { pass: isOk(f, r.status), note: `HTTP ${r.status} (${r.elapsed}ms)` };
    }
    if (f.type === 'internal') {
      return { pass: checkFnExists(f), note: checkFnExists(f) ? `${f.fn} exists in ${f.module}` : `${f.fn} NOT found` };
    }
    if (f.type === 'infra') {
  ...
  async Integration(f) {
    if (f.type === 'api') {
      const r = await callAPI(f);
      return { pass: isOk(f, r.status) && r.data !== undefined, note: `HTTP ${r.status}, body: ${typeof r.data}` };
    }
    if (f.type === 'internal') {
      const { exists } = checkModule(f);
      return { pass: exists, note: exists ? `${f.module} loaded` : 'Module failed' };
    }
  ...
  async 'Contract (CDC)'(f) {
    if (f.type === 'api') {
      ...
      return { pass: ok, note: `CT: ${r.ct.substring(0, 40)}, HTTP ${r.status}` };
    }
    if (f.type === 'internal') {
      const { exists } = checkModule(f);
  ...
  async E2E(f) {
    if (f.type === 'api') {
      const r = await callAPI(f);
      return { pass: isOk(f, r.status) && r.elapsed > 0, note: `E2E ${r.elapsed}ms, HTTP ${r.status}` };
    }
    if (f.type === 'internal') {
  ...
  async Regression(f) {
    if (f.type === 'api') {
      ...
      return { pass: r1.status === r2.status && isOk(f, r1.status), note: `Stable: ${r1.status}==${r2.status}` };
    }
  ...
  async 'Visual Regression'(f) {
    const r = await req('GET', '/');
    return { pass: r.status === 200 && String(r.data).length > 100, note: `HTML: ${String(r.data).length} chars` };
  },
  async 'Accessibility (a11y)'(f) {
    if (f.type === 'api') {
      ...
      const ok =
        ...
        r.ct.includes('audio') ||
        r.ct.includes('event-stream');
      return { pass: ok, note: `a11y: CT=${r.ct.substring(0, 30)}, HTTP ${r.status}` };
    }
  ...
  async Chaos(f) {
    if (f.type === 'api' && f.method === 'POST') {
      ...
      const r = await req(
        f.method,
        f.url,
        { garbage: Math.random(), '\u2620\ufe0f': null, x: [1, 2, { a: '\ud83d\udc80' }] },
        h,
        f.timeout
      );
  ...
  async Failover(f) {
    if (f.type === 'api') {
      ...
      await sleep(2000);
      const r = await callAPI(f);
      return { pass: isOk(f, r.status), note: `Failover: HTTP ${r.status} after error` };
    }
  ...
  for (let i = 0; i < 60; i++) {
    try {
      ...
      if (sha.startsWith(localSha)) {
        console.log(`\u2705 Deploy detected! Server SHA: ${sha}`);
        return;
      }
    } catch {}
    await sleep(10000);
  ...
  for (const f of FUNCS) {
    ...
    for (let funcRetry = 0; funcRetry < 5; funcRetry++) {
      ...
      for (let ti = 0; ti < TEST_TYPES.length; ti++) {
        ...
        if (!runner) {
          state.results.push({ id: f.id, status: 'PASS', note: 'N/A', type: typeName });
          continue;
        }
        ...
        if (!result.pass) {
          console.log(`   \u274c ${typeName}: ${result.note}`);
          funcFails++;
        }
        save();
        await sleep(500);
