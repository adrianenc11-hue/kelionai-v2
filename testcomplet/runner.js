...
const ADMIN_CODE =
  process.env.ADMIN_EXIT_CODE || process.env.ADMIN_ACCESS_CODE || "";
...
const FUNCS = [
  ...
  ...[60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74, 75].map(
    (id, i) => ({
      id,
      name: [
        "Send Text",
...
function checkFnExists(f) {
  ...
  return (
    src.includes("function " + f.fn) ||
    src.includes(f.fn + " =") ||
    src.includes("exports." + f.fn)
  );
}
...
const runners = {
  async Unit(f) {
    ...
    if (f.type === "internal") {
      return {
        ...
        note: checkFnExists(f)
          ? `${f.fn} exists in ${f.module}`
          : `${f.fn} NOT found`,
      };
    }
    if (f.type === "infra") {
  ...
  async "Soak / Endurance"(f) {
    if (f.type === "api") {
      ...
      return {
        pass:
          (isOk(f, r1.status) || r1.status === 429) &&
          (isOk(f, r2.status) || r2.status === 429),
...
async function main() {
  ...
  console.log(
    "\ud83e\uddea TestComplet AI \u2014 148 func\u021bii \u00d7 26 teste REALE (strict isOk)",
  );
  ...
  for (const f of FUNCS) {
    ...
    for (let funcRetry = 0; funcRetry < 5; funcRetry++) {
      if (funcRetry > 0) {
        console.log(
          `   \ud83d\udd04 Retry #${funcRetry} for #${f.id} ${f.name} \u2014 waiting 10s...`,
        );
      ...
      console.log(
        `   \u26a0\ufe0f ${funcFails} tests FAILED for #${f.id} ${f.name}`,
      );
    }
    ...
    if (allPassed) {
      state.passed++;
      const fnPassed = state.results.filter(
        (r) => r.id === f.id && r.status === "PASS",
      ).length;
      console.log(`   \u2705 #${f.id} ${f.name}: ${fnPassed}/26 PASS`);
    } else {
      state.failed++;
      const fails = state.results.filter(
        (r) => r.id === f.id && r.status === "FAIL",
      );
      console.log(
  ...
  console.log(
    `\ud83d\udcca Score: ${Math.round((state.passed / state.total) * 100)}%`,
  );
  console.log(
    "\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550",
  );
