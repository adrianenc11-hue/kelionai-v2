(function () {
  ...
  async function renderPricing() {
    ...
    var usage =
      status && status.usage ? status.usage : { chat: 0, search: 0, image: 0 };
    ...
    if (plans.length === 0) {
      grid.innerHTML =
        '<div class="pricing-loading">Plans are not available at the moment.</div>';
      return;
    }
    ...
    for (var i = 0; i < plans.length; i++) {
      ...
      if (isCurrent) {
        actionDiv.innerHTML =
          '<button class="pricing-btn current" disabled>Current plan</button>';
      } else if (p.price === 0) {
    ...
    if (
      ...
      manageBtn.innerHTML =
        '<button class="pricing-btn manage" id="btn-manage-sub">Manage subscription</button>';
      grid.parentNode.appendChild(manageBtn);
      document
        .getElementById("btn-manage-sub")
        .addEventListener("click", KShared.openPortal);
    }
  }
  ...
  function checkPaymentResult() {
    ...
    if (payment === "success") {
      setTimeout(function () {
        alert(
          "✅ Payment processed successfully! Your plan has been activated.",
        );
      }, 500);
  ...
  function init() {
    var pricingBtn =
      document.getElementById("btn-pricing") ||
      document.getElementById("btn-subscriptions");
    var modal = document.getElementById("pricing-modal");
