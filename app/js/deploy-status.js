// ═══════════════════════════════════════════════════════════════
// KelionAI — Deploy Status Indicator
...
(function () {
  ...
  function createOverlay() {
    ...
    overlay.innerHTML = `
      ...
      <div class="deploy-title">Deploy în curs<span class="deploy-dots"></span></div>
      <div class="deploy-subtitle">Se așteaptă repornirea serverului...</div>
  ...
  function showError(msg) {
    ...
    overlay.querySelector('.deploy-subtitle').textContent = msg || 'Serverul nu a răspuns la timp.';
    setTimeout(function () {
      if (overlay) {
        overlay.classList.remove('visible');
        setTimeout(function () {
  ...
  async function pollHealth() {
    ...
    if (Date.now() - startTime > MAX_WAIT) {
      ...
      showError('Timeout — serverul nu a revenit în 5 minute.');
      return;
    }
    ...
    try {
      ...
      var subtitleEl = overlay ? overlay.querySelector('.deploy-subtitle') : null;
      if (subtitleEl) subtitleEl.textContent = 'Serverul se repornește...';
