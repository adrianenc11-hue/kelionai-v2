...
(function () {
  ...
  function _ensureCCElement() {
    ...
    var parent =
      document.getElementById("avatar-area") ||
  ...
  function _showCC(text) {
    ...
    _ccBuffer += text;
    var display =
      _ccBuffer.length > 150 ? "..." + _ccBuffer.slice(-147) : _ccBuffer;
  ...
  function _activateFallback(reason) {
    ...
    window.dispatchEvent(
      new CustomEvent("voicefirst-fallback", { detail: { reason: reason } }),
    );
  }
  ...
  function connect(avatar, language) {
    ...
    socket.on("ready", function (data) {
      ...
      window.dispatchEvent(
        new CustomEvent("voicefirst-ready", { detail: data }),
      );
      startMic();
    });
    ...
    socket.on("transcript", function (data) {
      ...
      window.dispatchEvent(
        new CustomEvent("voicefirst-transcript", { detail: data }),
      );
    });

    socket.on("transcript_done", function (data) {
      window.dispatchEvent(
        new CustomEvent("voicefirst-transcript-done", { detail: data }),
      );
    });
    ...
    socket.on("turn_complete", function (data) {
      _hideCC();
      window.dispatchEvent(
        new CustomEvent("voicefirst-turn-complete", { detail: data }),
      );
    });
    ...
    socket.on("error_msg", function (data) {
      ...
      window.dispatchEvent(
        new CustomEvent("voicefirst-error", { detail: data }),
      );
    });
  ...
  function _queueAudio(base64Audio) {
    ...
    window.dispatchEvent(
      new CustomEvent("audio-start", {
        detail: { duration: audioBuf.duration, isRealtime: true },
      }),
    );
  }
