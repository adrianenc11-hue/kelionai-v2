...
(function () {
  ...
  // ══════════════════════════════════════════════════════════
  // LIVE SUBTITLE — afiseaza conversatia sub avatar
  // User vorbeste → text user; Avatar raspunde → inlocuieste cu text AI
  // ══════════════════════════════════════════════════════════
  var _subtitleClearTimer = null;
  var _subtitleEl = null;
  var _subtitleSpeaker = null;
  var _subtitleTxt = null;

  function _getSubtitleEls() {
    if (!_subtitleEl) _subtitleEl = document.getElementById('live-subtitle');
    if (!_subtitleSpeaker) _subtitleSpeaker = document.getElementById('subtitle-speaker');
    if (!_subtitleTxt) _subtitleTxt = document.getElementById('subtitle-text');
    return _subtitleEl && _subtitleSpeaker && _subtitleTxt;
  }

  function showLiveSubtitle(role, text) {
    if (!_getSubtitleEls()) return;
    if (_subtitleClearTimer) { clearTimeout(_subtitleClearTimer); _subtitleClearTimer = null; }

    _subtitleEl.className = 'visible ' + (role === 'user' ? 'role-user' : 'role-ai');
    _subtitleSpeaker.textContent = role === 'user' ? '🎤' : '🔊';

    // Afiseaza maxim 200 caractere
    var display = text.length > 200 ? text.slice(0, 200) + '…' : text;
    _subtitleTxt.textContent = display;

    // User text: dispare cand avatarul incepe sa raspunda
    // AI text: dispare dupa 4s (gestionat in audio_end)
    if (role === 'user') {
      if (_subtitleClearTimer) clearTimeout(_subtitleClearTimer);
      _subtitleClearTimer = setTimeout(function () {
        hideLiveSubtitle();
      }, 8000);
    }
  }

  function showLiveSubtitleState(state) {
    if (!_getSubtitleEls()) return;
    if (state === 'listening') {
      // Curata si pregateste pentru urmatoarea intrare
      if (_subtitleClearTimer) { clearTimeout(_subtitleClearTimer); _subtitleClearTimer = null; }
      _subtitleEl.className = 'visible role-listening';
      _subtitleSpeaker.textContent = '🎤';
      _subtitleTxt.textContent = '…';
    } else if (state === 'thinking') {
      _subtitleEl.className = 'visible role-thinking';
      _subtitleSpeaker.textContent = '💭';
      _subtitleTxt.textContent = '…';
    }
  }
    ...
    switch (msg.type) {
      ...
      case 'audio_end':
        ...
        setTimeout(function () {
          ...
          // Sterge subtitlul dupa 4s (da timp utilizatorului sa citeasca)
          if (_subtitleClearTimer) clearTimeout(_subtitleClearTimer);
          _subtitleClearTimer = setTimeout(function () {
            hideLiveSubtitle();
          }, 4000);
      ...
      case 'cc':
        // ── Afiseaza conversatia NUMAI sub avatar (#live-subtitle) ──
        showLiveSubtitle(msg.role, msg.text || '');
        window.dispatchEvent(new CustomEvent('live-voice-cc', { detail: msg }));
        break;

      case 'cc_done':
        // Textul ramine vizibil — nu il sterge la cc_done
        console.log('[LiveVoice] ' + (msg.role === 'user' ? 'TU' : 'AI') + ': ' + (msg.text || '').substring(0, 80));
        break;

      case 'speech_started':
        if (window.KAvatar) try { KAvatar.setExpression('thinking', 0.3); } catch (_e) {}
        // Arata ca userul vorbeste (indicator mic activ)
        showLiveSubtitleState('listening');
        emitState('user_speaking');
        break;

      case 'speech_stopped':
        if (window.KAvatar) try { KAvatar.setExpression('thinking', 0.5); } catch (_e) {}
        // Userul a terminat — avatarul gandeste
        showLiveSubtitleState('thinking');
        emitState('thinking');
        break;
  ...
  function toggle() {
    if (_active || _connecting) { disconnect(); return false; }
    connect();
    return true;
  }
