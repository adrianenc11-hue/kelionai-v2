...
(function () {
  ...
  (function _createSlider() {
    ...
    wrap.style.cssText =
      'position:fixed;bottom:12px;right:12px;z-index:99999;background:rgba(0,0,0,0.75);padding:8px 14px;border-radius:10px;font-family:sans-serif;font-size:12px;color:#fff;display:flex;align-items:center;gap:8px;';
  ...
  SimpleLipSync.prototype.update = function () {
    ...
    function clamp(v, max) {
      return Math.min(v * _mouthMultiplier, (max || MAX_VISEME) * _mouthMultiplier);
    }
    ...
    var visemes = {
      ...
      viseme_aa: clamp(low * 1.2, MAX_VISEME_AA),
      aa: clamp(low * 1.2, MAX_VISEME_AA),
      viseme_O: clamp(low * 0.9 * (1 - mid * 0.3)),
      oh: clamp(low * 0.9 * (1 - mid * 0.3)),
      viseme_E: clamp(mid * 1.0 * (1 - low * 0.2)),
      E: clamp(mid * 1.0 * (1 - low * 0.2)),
      viseme_I: clamp(mid * 0.8 * (1 - low * 0.4)),
      ih: clamp(mid * 0.8 * (1 - low * 0.4)),
      viseme_U: clamp(low * 0.6 + mid * 0.3),
      ou: clamp(low * 0.6 + mid * 0.3),

      // Consonants — driven by mid + hi (both naming conventions)
      viseme_PP: mid > 0.3 ? clamp((1 - low) * 0.5) : 0,
      PP: mid > 0.3 ? clamp((1 - low) * 0.5) : 0,
      viseme_FF: clamp(hi * 0.8),
      FF: clamp(hi * 0.8),
      viseme_TH: clamp(hi * 0.5 + mid * 0.3),
      TH: clamp(hi * 0.5 + mid * 0.3),
      viseme_DD: clamp(mid * 0.6 + low * 0.2),
      DD: clamp(mid * 0.6 + low * 0.2),
      viseme_kk: clamp(mid * 0.5 * (1 - hi * 0.3)),
      kk: clamp(mid * 0.5 * (1 - hi * 0.3)),
      viseme_CH: clamp(hi * 0.9),
      CH: clamp(hi * 0.9),
      viseme_SS: clamp(hi * 1.0),
      SS: clamp(hi * 1.0),
      viseme_nn: clamp(mid * 0.5),
      nn: clamp(mid * 0.5),
      viseme_RR: clamp(mid * 0.7 + low * 0.3),
      RR: clamp(mid * 0.7 + low * 0.3),
      viseme_sil: 0,
      sil: 0,
      ...
      mouthRollUpper: clamp(mid * 0.1),
      mouthShrugLower: clamp(low * 0.2 * (1 - mid)),
  ...
  var PHONEME_VISEME = {
    a: { viseme_aa: 0.2, jawOpen: 0.15 },
    ă: { viseme_aa: 0.12, jawOpen: 0.1 },
    â: { viseme_aa: 0.1, jawOpen: 0.08 },
    î: { viseme_I: 0.12, jawOpen: 0.05 },
    e: { viseme_E: 0.18, jawOpen: 0.08 },
    i: { viseme_I: 0.12, jawOpen: 0.04 },
    o: { viseme_O: 0.18, mouthFunnel: 0.1, jawOpen: 0.1 },
    u: { viseme_U: 0.15, mouthPucker: 0.1, jawOpen: 0.05 },
    b: { viseme_PP: 0.15 },
    p: { viseme_PP: 0.15 },
    m: { viseme_PP: 0.12 },
    f: { viseme_FF: 0.15 },
    ...
    z: { viseme_SS: 0.1 },
    ș: { viseme_CH: 0.12 },
    ț: { viseme_SS: 0.08, viseme_DD: 0.08 },
    t: { viseme_DD: 0.1 },
    d: { viseme_DD: 0.1 },
    n: { viseme_nn: 0.12 },
    l: { viseme_nn: 0.08, jawOpen: 0.05 },
