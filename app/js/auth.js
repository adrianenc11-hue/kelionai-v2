(function () {
  ...
  function saveSession(s, u) {
    if (s) {
      ...
      if (s.refresh_token)
        sessionStorage.setItem("kelion_refresh_token", s.refresh_token);
      if (s.expires_at)
        sessionStorage.setItem("kelion_token_expires", s.expires_at);
    }
  ...
  async function login(email, pw) {
    ...
    try {
      ...
      console.warn(
        "[Auth] First login attempt failed, retrying...",
        firstErr.message,
      );
        ...
        try {
          if (isReg) {
            ...
            if (err) {
              err.style.color = "#00ff88";
              err.textContent =
                d.message || "Please check your email to verify your account.";
            }
          } else {
            await login(em, pw);
  ...
  async function init() {
    ...
    if (
      inviteCode &&
      /^KEL-[0-9a-fA-F]{4}-[0-9a-fA-F]{6}-[A-Z0-9]{10}$/i.test(inviteCode)
    ) {
      ...
      if (authScreen && !authScreen.classList.contains("hidden")) {
        ...
        if (!badge) {
          ...
          badge.textContent =
            "🎁 Invitation from a friend! +5 bonus days on your first subscription";
