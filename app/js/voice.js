...
(function () {
    ...
    recognition.onresult = (event) => {
      ...
      for (let i = event.resultIndex; i < event.results.length; i++) {
        ...
        if ((hasKelion || hasKira) && t.length > 1) {
          ...
          if (
            window.KAvatar &&
            targetAvatar !== window.KAvatar.getCurrentAvatar()
          ) {
            window.KAvatar.loadAvatar(targetAvatar);
            document
              .querySelectorAll(".avatar-pill")
              .forEach((b) =>
                b.classList.toggle("active", b.dataset.avatar === targetAvatar),
              );
            const displayName =
              targetAvatar.charAt(0).toUpperCase() + targetAvatar.slice(1);
  ...
  function _chunkText(text) {
    ...
    for (let i = 0; i < sentences.length; i++) {
      if (
        current.length + sentences[i].length > CHUNK_MAX &&
        current.length > 0
      ) {
      ...
      function (audioBuf) {
        ...
        if (alignment && window.AlignmentLipSync) {
          try {
            ...
            console.warn(
              "[Voice] AlignmentLipSync failed, falling back to FFT:",
              e.message,
            );
          }
        }
  ...
  async function playAudioBuffer(arrayBuf, fallbackText) {
    ...
    if (fallbackText) {
      ...
      if (
        /\b(nu|no|nein|non|niet|imposibil|impossible|unfortunately|din păcate)\b/i.test(
          gt,
        )
      ) {
        setTimeout(function () {
          ...
          function updateMicLevel() {
            ...
            if (vol > 0.05) {
              ...
              for (let k = 0; k < bars.length; k++) {
                const h = Math.max(
                  4,
                  Math.min(22, vol * 22 * (1 + Math.random() * 0.3)),
                );
