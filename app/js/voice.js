...
(function () {
    ...
    async function playAudioBuffer(arrayBuf, fallbackText) {
        ...
        if (fallbackText) {
            var lowerText = fallbackText.toLowerCase();
            setTimeout(function() { if (window.KAvatar) KAvatar.playGesture('nod'); }, 500);
            if (lowerText.includes('?')) setTimeout(function() { if (window.KAvatar) KAvatar.playGesture('tilt'); }, 2000);
            if (lowerText.includes('!')) setTimeout(function() { if (window.KAvatar) KAvatar.playGesture('nod'); }, 1500);
            ...
            if (/\b(nu|no|nein|non|niet|imposibil|impossible|unfortunately|din păcate)\b/i.test(lowerText)) {
