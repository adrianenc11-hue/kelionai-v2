(function () {
  ...
  function renderPlans(plans, status) {
    ...
    if (!plans || plans.length === 0) {
      container.innerHTML =
        '<div class="plan-loading">Plans are not available at this time.</div>';
      return;
    }
    ...
    filtered.forEach(function (p) {
      ...
      if (p.savings) {
        badgeHtml +=
          '<div class="savings-badge">' + KShared.esc(p.savings) + "</div>";
      }
      ...
      var featuresHtml = (p.features || [])
        .map(function (f) {
          return (
            '<li><span class="feature-check">✓</span> ' +
            KShared.esc(f) +
            "</li>"
          );
        })
      ...
      if (isCurrent) {
        ...
        if (subscription && subscription.current_period_end) {
          ...
          btnHtml +=
            '<button class="plan-btn manage" id="btn-manage-' +
            KShared.esc(p.id) +
        ...
        btnHtml =
          '<button class="plan-btn free-plan" disabled>Included</button>';
      } else {
        btnHtml =
  ...
  function showStatus(status) {
    ...
    if (plan !== "guest" && plan !== "free") {
      ...
      msg =
        "✅ Current plan: <strong>" +
        KShared.esc(plan.toUpperCase()) +
        "</strong>" +
        renewDate;
    } else if (plan === "free") {
      msg =
        "You are on the <strong>Free</strong> plan. Upgrade for extended access.";
    }
