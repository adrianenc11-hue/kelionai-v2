...
function hdrs() {
  ...
  try {
    let t = null;
    // Priority 1: localStorage kelion_token (set by auth.js on login)
    t = localStorage.getItem('kelion_token');
...
async function loadClients() {
  try {
    ...
    if (_clients.length === 0) {
      tbody.innerHTML = '<tr><td colspan="6" style="color:#888">Niciun client</td></tr>';
      return;
    }
...
async function loadCodes() {
  try {
    ...
    if (codes.length === 0) {
      tbody.innerHTML = '<tr><td colspan="6" style="color:#888">Niciun cod</td></tr>';
      return;
    }
...
async function loadAudit() {
  ...
  try {
    ...
    el.innerHTML = '<div style="color:#f87171">Error loading audit: ' + e.message + '</div>';
  }
}
    ...
    for (let i = 0; i < keys.length; i++) {
      try {
        ...
        if (raw) {
          const parsed = JSON.parse(raw);
          if (parsed && parsed.access_token) {
            localStorage.setItem('kelion_token', parsed.access_token);
            if (parsed.refresh_token) localStorage.setItem('kelion_refresh_token', parsed.refresh_token);
            if (parsed.expires_at) localStorage.setItem('kelion_token_expires', String(parsed.expires_at));
            if (parsed.user) localStorage.setItem('kelion_user', JSON.stringify(parsed.user));
            console.log('[Admin] ✅ User session synced to localStorage');
            break;
          }
        }
      } catch (_e2) {
