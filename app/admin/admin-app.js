...
async function loadAiStatus() {
  try {
    ...
    (d.providers || []).forEach(function (p) {
      ...
      const projStr =
        p.projectedMonth > 0 ? "~$" + p.projectedMonth.toFixed(2) + "/mo" : "";
      const alertDot =
        p.alertLevel === "red" ? "🔴" : p.alertLevel === "yellow" ? "🟡" : "🟢";
      ...
      card.addEventListener("click", function () {
        ...
        let details =
          ...
          "</div>";
        details +=
          "<div>📊 Consum luna: <strong>" +
          costStr +
          "</strong> (" +
          reqStr +
          ")</div>";
        if (p.projectedMonth > 0)
          details +=
            "<div>📈 Proiecție: ~$" +
            p.projectedMonth.toFixed(4) +
            "/lună</div>";
        if (p.freeQuota > 0)
          details +=
            "<div>🎁 Cotă gratuită: " +
    ...
    document.getElementById("ai-status-grid").innerHTML =
...
async function loadTraffic() {
  try {
    ...
    document.getElementById("traffic-active").textContent =
      d.activeConnections || 0;
    // Show all-time total in stats bar (real count from DB, excluding internal IPs)
    document.getElementById("val-views-today").textContent =
      d.totalAllTime || d.totalToday || 0;
...
async function loadCredit() {
  try {
    ...
    if (d.byProvider && d.byProvider.length > 0) {
      ...
      tbody.innerHTML =
        '<tr><td colspan="6" style="color:#888;text-align:center">No cost data this month</td></tr>';
    }
    if (d.totalMonth > 0) {
      const el = document.getElementById("credit-distribution");
      let distHtml =
        '<h4 style="margin:12px 0 8px;color:#06b6d4">Distribuție costuri:</h4>';
    ...
    if (tb)
      tb.innerHTML =
        '<tr><td colspan="6" style="color:#f66;text-align:center">Error loading costs</td></tr>';
  }
}
...
async function loadClients() {
  try {
    ...
    if (_clients.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="color:#888">Niciun client</td></tr>';
      return;
    }
    tbody.innerHTML = _clients
      .map(function (u) {
...
async function loadCodes() {
  try {
    ...
    if (codes.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="6" style="color:#888">Niciun cod</td></tr>';
      return;
    }
...
async function loadAudit() {
  ...
  try {
    ...
    el.innerHTML =
      '<div style="color:#f87171">Error loading audit: ' + e.message + "</div>";
  }
}
...
async function _runAutoFix() {
  if (
    !confirm(
      "Brain-ul va înlocui automat valorile hardcodate cu process.env.APP_URL. Continui?",
    )
  )
    return;
  try {
...
async function loadMedia() {
  try {
    ...
    document.getElementById("media-images").textContent =
      (stats.image || 0) + (stats.vision || 0);
    ...
    const otherCount =
      ...
      (stats.vision || 0) -
      (stats.tts || 0);
    document.getElementById("media-other").textContent = Math.max(
      0,
      otherCount,
    );
    ...
    if (!d.recent || d.recent.length === 0) {
      const msg =
        d.totalCount > 0
          ? d.totalCount +
            " records in database (details restricted by row-level security)"
    ...
    tb.innerHTML = d.recent
      .map(function (m) {
        ...
        const urlCell = m.url
          ? '<a href="' +
            esc(m.url) +
            '" target="_blank" style="color:#06B6D4">View</a>'
          : "—";
        const prompt = m.prompt
          ? esc(m.prompt).substring(0, 60) + (m.prompt.length > 60 ? "…" : "")
          : "—";
        return (
          "<tr><td>" +
          esc(date) +
...
async function loadTrading() {
  try {
    ...
    document.getElementById("trade-winrate").textContent =
      (s.winRate || 0) + "%";
    ...
    if (!d.recentTrades || d.recentTrades.length === 0) {
      tb.innerHTML =
        '<tr><td colspan="7" style="color:#666;text-align:center">No trades yet</td></tr>';
    } else {
      tb.innerHTML = d.recentTrades
        .slice(0, 20)
        .map(function (t) {
    ...
    if (!d.intelligence || d.intelligence.length === 0) {
      ib.innerHTML =
        '<tr><td colspan="5" style="color:#666;text-align:center">No intelligence data yet</td></tr>';
    } else {
      ib.innerHTML = d.intelligence
        .slice(0, 10)
        .map(function (i) {
          ...
          const analysis = i.analysis
            ? esc(String(i.analysis)).substring(0, 80) + "…"
            : "—";
          return (
            "<tr><td>" +
            esc(date) +
    ...
    document.getElementById("trading-tbody").innerHTML =
...
async function loadMemories(typeFilter, searchQuery) {
  try {
    ...
    if (!d.memories || d.memories.length === 0) {
      tb.innerHTML =
        '<tr><td colspan="5" style="color:#888;text-align:center">No memories found</td></tr>';
    } else {
    ...
    if (ftb) {
      if (!d.facts || d.facts.length === 0) {
        ftb.innerHTML =
          '<tr><td colspan="4" style="color:#888;text-align:center">No facts learned yet</td></tr>';
      } else {
        ftb.innerHTML = d.facts
          .map(function (f) {
    ...
    if (tb)
      tb.innerHTML =
        '<tr><td colspan="5" style="color:#f66">Error: ' +
        e.message +
        "</td></tr>";
  }
}
...
async function loadBrain() {
  try {
    ...
    document.getElementById("brain-uptime").textContent =
      uptH + "h " + uptM + "m";

    // Conversations (from DB — persists across deploys)
    document.getElementById("brain-conversations").textContent =
    ...
    if (toolsEl && d.toolStats) {
      toolsEl.innerHTML = Object.entries(d.toolStats)
        .map(function (kv) {
          const icon = kv[1] > 0 ? "🟢" : "⚪";
          return (
            '<span class="badge ' +
    ...
    if (keysEl && d.providers) {
      keysEl.innerHTML = Object.entries(d.providers)
        .map(function (kv) {
          const icon = kv[1] ? "🟢" : "🔴";
          return (
            '<span class="badge ' +
...
async function initAdmin() {
  ...
  const currentSecret =
    window.adminSecret || sessionStorage.getItem("kelion_admin_secret") || "";
          ...
          if (parsed && parsed.access_token) {
            ...
            if (parsed.refresh_token)
              sessionStorage.setItem(
                "kelion_refresh_token",
                parsed.refresh_token,
              );
            if (parsed.expires_at)
              sessionStorage.setItem(
                "kelion_token_expires",
...
function renderNotifList() {
  ...
  if (_notifs.length === 0) {
    el.innerHTML =
      '<div style="padding:12px;text-align:center;color:#666">No notifications</div>';
    return;
  }
  const colors = {
    info: "#3b82f6",
