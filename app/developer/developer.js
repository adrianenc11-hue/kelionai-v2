...
$("auth-toggle-link").addEventListener("click", () => {
  ...
  $("auth-toggle-link").textContent = isRegister
    ? "Sign in instead"
    : "Create one";
  document.querySelector("#auth-switch").firstChild.textContent = isRegister
...
$("auth-submit-btn").addEventListener("click", async () => {
  ...
  if (!email || !password) {
    $("auth-err").textContent = "Email and password required";
    return;
  }
...
function renderKeyList(keys) {
  ...
  if (!keys || !keys.length) {
    el.innerHTML =
      '<div class="empty-state"><div class="icon">🔑</div><p>No API keys yet. Generate one above.</p></div>';
    return;
  }
  el.innerHTML = keys
    .map(
...
$("create-key-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) {
      showAlert("key-alert", data.error || "Failed to create key", "error");
      return;
    }
    ...
    showAlert("key-alert", "Network error", "error");
  } finally {
...
$("revoke-confirm-btn").addEventListener("click", async () => {
  ...
  closeRevokeModal();
  try {
    const res = await apiFetch("/api/developer/keys/" + id, {
      method: "DELETE",
    });
    if (res.ok) {
...
$("save-webhook-btn").addEventListener("click", async () => {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) {
      showAlert("webhook-alert", data.error || "Failed to save", "error");
      return;
    }
    showAlert(
      "webhook-alert",
      url ? "Webhook saved!" : "Webhook removed.",
      "success",
    );
  } catch (_) {
    showAlert("webhook-alert", "Network error", "error");
  } finally {
    $("save-webhook-btn").disabled = false;
  }
});
