...
describe('verifyReferralCode', () => {
  ...
  test('detects expired code (timestamp > CODE_EXPIRY_DAYS old)', () => {
    ...
    const secret = process.env.REFERRAL_SECRET || process.env.SESSION_SECRET || 'kelion-referral-dev-only';
    const hmac = crypto.createHmac('sha256', secret).update(payload).digest('hex').slice(0, 10).toUpperCase();
...
describe('applyReferralBonus', () => {
  function mockSupabase(opts = {}) {
    const { refCodeData = null, existingSubscription = null, updateResult: _updateResult = {} } = opts;
    ...
    return {
      ...
      from: (table) => {
        if (table === 'referral_codes') {
          return {
            select: () => ({
              eq: () => ({ single: async () => ({ data: refCodeData }) }),
            }),
            update: (data) => {
