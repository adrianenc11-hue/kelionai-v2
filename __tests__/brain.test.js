...
describe("KelionBrain", () => {
  ...
  describe("refineSearchQuery", () => {
    test("removes filler words from queries", () => {
      const result = brain.refineSearchQuery(
        "te rog spune-mi despre quantum computing",
      );
    ...
    test("truncates very long queries", () => {
      const longQ =
        "tell me everything you know about the history of artificial intelligence from the very beginning until now including all major milestones";
...
describe("analyzeIntent — Complexity", () => {
  ...
  test("request multi-tool → complex", () => {
    const r = brain.analyzeIntent(
      "caută hoteluri în Paris și arată-mi vremea și generează o imagine cu Turnul Eiffel",
      "ro",
    );
...
describe("buildPlan", () => {
  ...
  test("plan cu search pentru query de căutare", () => {
    const analysis = brain.analyzeIntent(
      "caută informații despre Python",
      "ro",
    );
    const plan = brain.buildPlan([{ analysis }], null);
...
describe("refineSearchQuery", () => {
  ...
  test("trunchiază query lung la 8 cuvinte", () => {
    const longQuery =
      "un doi trei patru cinci sase sapte opt noua zece unsprezece doisprezece treisprezece paisprezece cincisprezece";
...
describe("decomposeTask", () => {
  ...
  test('descompune request cu "și" în 2+ subtask-uri', async () => {
    ...
    const tasks = await brain.decomposeTask(
      "caută hoteluri și arată meteo",
      analysis,
      "ro",
    );
