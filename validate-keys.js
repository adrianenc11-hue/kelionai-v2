...
function log(name, status, detail) {
  ...
  console.log(
    `${icon} ${name.padEnd(30)} ${status.padEnd(10)} ${detail || ""}`,
  );
}
...
async function run() {
  console.log("\n═══════════════════════════════════════════════════════════");
  console.log("  KelionAI — KEY VALIDATOR (Live API Tests)");
  console.log("═══════════════════════════════════════════════════════════\n");
  ...
  await testKey("GOOGLE_AI_KEY", async (key) => {
    ...
    if (r.status === 400 || r.status === 403)
      return { ok: false, detail: "INVALID KEY or API not enabled" };
    if (r.status === 429)
      return { ok: true, detail: "Rate limited but key valid" };
  ...
  await testKey("SUPABASE_URL", async (val) => {
    ...
    if (r.status === 200 || r.status === 401)
      return { ok: true, detail: `Supabase reachable (${r.status})` };
  ...
  await testKey("SUPABASE_ANON_KEY", async (key) => {
    ...
    if (!url)
      return { ok: false, detail: "SUPABASE_URL not set — cannot test" };
    const r = await fetch(`${url}/rest/v1/`, { headers: { apikey: key } });
    if (r.status === 200) return { ok: true, detail: "Anon key valid" };
    if (r.status === 401 || r.status === 403)
      return { ok: false, detail: "Key rejected (expired or wrong project)" };
  ...
  await testKey("SUPABASE_SERVICE_KEY", async (key) => {
    ...
    if (r.status === 200)
      return { ok: true, detail: "Service key valid (admin access)" };
    return { ok: false, detail: `HTTP ${r.status}` };
  });
  ...
  await testKey("ELEVENLABS_API_KEY", async (key) => {
    ...
    if (r.status === 401)
      return { ok: false, detail: "INVALID or EXPIRED key" };
    return { ok: false, detail: `HTTP ${r.status}` };
  });
  ...
  await testKey("STRIPE_SECRET_KEY", async (key) => {
    if (!key.startsWith("sk_"))
      return { ok: false, detail: "Must start with sk_live_ or sk_test_" };
  ...
  console.log("\n── 🔍 EXTRA (în cod dar nu în lista ta) ──\n");
  ...
  const ok = results.filter(
    (r) => r.status === "OK" || r.status === "SET",
  ).length;
  const missing = results.filter((r) => r.status === "MISSING").length;
  const expired = results.filter(
    (r) =>
      r.status === "EXPIRED" || r.status === "INVALID" || r.status === "ERROR",
  ).length;
  console.log(
    `  ✅ OK/SET: ${ok}   ⬜ MISSING: ${missing}   🔴 EXPIRED/INVALID: ${expired}`,
  );
  ...
  console.log(
    "  Code: MESSENGER_PAGE_TOKEN → List: MESSENGER_PAGE_TOKEN ✅ match",
  );
