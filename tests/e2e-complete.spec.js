...
test.beforeAll(async ({ request }) => {
    try {
        const r = await request.get('https://kelionai.app/api/health', { timeout: 15000 });
        if (r.status() >= 500) siteIsUp = false;
    } catch { siteIsUp = false; }
});
...
test.describe('New Pages', () => {
    test.beforeEach(async () => { if (!siteIsUp) test.skip(); });
    ...
    test('#15 /error.html loads', async ({ page }) => {
        const resp = await page.goto('/error.html');
        await expect(page.locator('body')).toBeVisible();
    ...
    test('#17 /dashboard/trading.html loads', async ({ page }) => {
        await page.goto('/dashboard/trading.html');
        await expect(page.locator('body')).toBeVisible();
        await page.screenshot({ path: 'test-results/dashboard-trading.png' });
    });
    ...
    test.beforeEach(async ({ page }) => {
        ...
        try {
            ...
            if (await authScreen.isVisible({ timeout: 3000 }).catch(() => false)) {
                await page.evaluate(() => {
                    document.querySelectorAll('#auth-screen, .auth-overlay').forEach(el => { if (el) el.style.display = 'none'; });
                });
            }
        } catch { }
    });
...
test.describe('Avatar Kira', () => {
    ...
    test('#55 Kira TTS has different voice', async ({ request }) => {
        test.skip(!siteIsUp);
        const kelionR = await request.post('/api/speak', { data: { text: 'Hello', avatar: 'kelion' } });
        const kiraR = await request.post('/api/speak', { data: { text: 'Hello', avatar: 'kira' } });
    ...
    test('#56 Kira chat has different personality', async ({ request }) => {
        ...
        const r = await request.post('/api/chat', {
            data: { message: 'describe your personality in one sentence', avatar: 'kira', language: 'en' }
        });
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
            const d = await r.json();
...
test.describe('Voice TTS + STT', () => {
    test('#59 POST /api/speak returns audio', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/speak', { data: { text: 'Hello world', avatar: 'kelion' } });
    ...
    test('#60 POST /api/speak Kira avatar returns audio', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/speak', { data: { text: 'Hello from Kira', avatar: 'kira' } });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBe(200);
        const buf = await r.body();
        expect(buf.length).toBeGreaterThan(100);
    ...
    test('#61 POST /api/listen with text (WebSpeech fallback)', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/listen', { data: { text: 'Test input from browser' } });
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
            const d = await r.json();
    ...
    test('#65 Multiple TTS requests all return audio (no intermittent)', async ({ request }) => {
        ...
        const results = await Promise.all([
            ...
            request.post('/api/speak', { data: { text: 'Second', avatar: 'kelion' } }),
...
test.describe('Microphone & Camera UI', () => {
    test('#68 mic button toggles recording UI', async ({ page }) => {
        ...
        const mic = page.locator('#btn-mic, .mic-btn, [data-action="mic"], button[aria-label*="mic"], button[aria-label*="Mic"], .voice-btn, #microphone-btn').first();
    ...
    test('#72 POST /api/vision with base64 image', async ({ request }) => {
        ...
        const r = await request.post('/api/vision', { data: { image: tinyImage, avatar: 'kelion', language: 'en' } });
        if (r.status() === 503) { test.skip(); return; } // Vision unavailable
        expect(r.status()).toBeLessThan(500);
    });
});
...
test.describe.serial('Auth Extended', () => {
    ...
    test('#82 POST /api/auth/forgot-password valid email returns 200', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/auth/forgot-password', { data: { email: 'test@keliontest.com' } });
    ...
    test('#83 POST /api/auth/forgot-password nonexistent email returns 200', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/auth/forgot-password', { data: { email: 'nonexistent_xyz_123@nope.com' } });
        expect(r.status()).toBeLessThan(500);
    });

    test('#84 POST /api/auth/reset-password invalid token returns 401', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/auth/reset-password', { data: { access_token: 'invalid_fake_token', password: 'NewPass123!' } });
        expect(r.status()).toBeGreaterThanOrEqual(400);
    });
    ...
    test('#85 POST /api/auth/change-email with auth', async ({ request }) => {
        ...
        await request.post('/api/auth/register', { data: { email: TEST_EMAIL, password: TEST_PASS, name: TEST_NAME } });
        const loginR = await request.post('/api/auth/login', { data: { email: TEST_EMAIL, password: TEST_PASS } });
...
test.describe('AI Services Extended', () => {
    test('#117 POST /api/imagine returns full base64 image', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/imagine', { data: { prompt: 'a red circle on white background' } });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
            const d = await r.json();
    ...
    test('#118 AI knows creator is Adrian', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'Who created you? Who is your creator?', avatar: 'kelion', language: 'en' }
        });
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
            const d = await r.json();
...
test.describe('Weather', () => {
    test('#119 POST /api/weather valid city returns data', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/weather', { data: { city: 'București' } });
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
            const d = await r.json();
    ...
    test('#120 POST /api/weather unknown city returns 404', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/weather', { data: { city: 'Xyznonexistentcity99999' } });
...
test.describe('Ticker', () => {
    test('#160 POST /api/ticker/disable without auth returns 401', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/ticker/disable', { data: { disabled: true } });
...
test.describe('i18n', () => {
    ...
    test('#163 chat responds in selected language', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'Spune-mi ceva interesant', avatar: 'kelion', language: 'ro' }
        });
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
            const d = await r.json();
...
test.describe('Referral Extended', () => {
    test('#168 GET /api/referral/code with auth returns code', async ({ request }) => {
        ...
        await request.post('/api/auth/register', { data: { email, password: TEST_PASS, name: 'RefTest' } });
        const loginR = await request.post('/api/auth/login', { data: { email, password: TEST_PASS } });
        if (loginR.status() !== 200) { test.skip(); return; }
        const token = (await loginR.json()).session?.access_token;
        if (!token) { test.skip(); return; }
        const r = await request.get('/api/referral/code', { headers: { Authorization: `Bearer ${token}` } });
        expect(r.status()).toBeLessThan(500);
    });
...
test.describe('UI Layout', () => {
    test('#170 chat area has minimum height', async ({ page }) => {
        ...
        await page.addInitScript(() => { localStorage.setItem('kelion_onboarded', 'true'); });
        await page.goto('/');
        await page.waitForSelector('#chat-messages, .chat-area, .messages-container', { state: 'visible', timeout: 60000 }).catch(() => { });
        ...
        if (await chatArea.isVisible().catch(() => false)) {
            ...
            if (box) { expect(box.height).toBeGreaterThanOrEqual(200); }
        }
...
test.describe.serial('E2E Flows', () => {
    test('#173 full auth: register → login → chat → logout', async ({ request }) => {
        ...
        // Register
        const regR = await request.post('/api/auth/register', { data: { email, password: TEST_PASS, name: 'FlowTest' } });
        ...
        // Me
        const meR = await request.get('/api/auth/me', { headers: { Authorization: `Bearer ${token}` } });
        expect(meR.status()).toBe(200);
        // Logout
        const logoutR = await request.post('/api/auth/logout', { headers: { Authorization: `Bearer ${token}` } });
    ...
    test('#174 voice round-trip: TTS returns audio', async ({ request }) => {
        test.skip(!siteIsUp);
        // Text → TTS
        const ttsR = await request.post('/api/speak', { data: { text: 'Voice round trip test', avatar: 'kelion' } });
        if (ttsR.status() === 503) { test.skip(); return; }
        ...
        const sttR = await request.post('/api/listen', { data: { text: 'Voice round trip test' } });
        expect(sttR.status()).toBeLessThan(500);
    });

    test('#175 search + chat follow-up', async ({ request }) => {
        test.skip(!siteIsUp);
        const searchR = await request.post('/api/search', { data: { query: 'AI news 2026' } });
        if (searchR.status() === 503) { test.skip(); return; }
        expect(searchR.status()).toBeLessThan(500);
        const chatR = await request.post('/api/chat', {
            data: { message: 'Tell me the latest AI news', avatar: 'kelion', language: 'en' }
        });
        if (chatR.status() === 503) { test.skip(); return; }
        expect(chatR.status()).toBeLessThan(500);
        if (chatR.status() === 200) {
...
test.describe('AI Simulations — Role-Based', () => {
    test('#176 Professor: structured lesson', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'Make me a lesson about photosynthesis with clear sections', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#177 Presenter: presentation outline', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'Create a presentation outline about solar energy with slides', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#178 Sales agent: structured pitch', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'Sell me a smartphone. Use a professional sales framework.', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#179 Consultant: business plan structure', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'What is the structure of a good business plan? Be specific.', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#180 Researcher: cites sources', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'Research the impact of AI on education. Cite your sources.', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#182 Planner: real data vacation plan', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'Plan a 5-day vacation in Greece. Include real places and practical tips.', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#184 Search-first: thinkTime > 0 on role request', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'Be a financial advisor and tell me how to invest 10000 euros', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
...
test.describe('Brain Intelligence', () => {
    test('#185 chain-of-thought: thinkTime on complex request', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'What is the weather in Bucharest and what are the latest tech news?', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#191 multi-tool: weather + search in one query', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'How is the weather in Paris and what are the top restaurants there?', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
...
test.describe('Persona & Emotions', () => {
    test('#193 Truth Engine: admits not knowing', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'What is the exact population of the city of Xyznonexistent?', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#194 EQ sadness: emotional validation', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'I am feeling very sad today', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#195 EQ joy: celebrates with user', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'I just passed my exam! I am so happy!', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#199 Proactive: "going outside" triggers weather suggestion', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'I am going outside now', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#202 Kelion vs Kira personality differs', async ({ request }) => {
        ...
        const kelionR = await request.post('/api/chat', { data: { message: q, avatar: 'kelion', language: 'en' } });
        if (kelionR.status() === 503) { test.skip(); return; }
        const kiraR = await request.post('/api/chat', { data: { message: q, avatar: 'kira', language: 'en' } });
        if (kiraR.status() === 503) { test.skip(); return; }
        if (kelionR.status() === 200 && kiraR.status() === 200) {
...
test.describe('Persistent Memory', () => {
    test('#203 memory save via chat', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'Remember that my favorite programming language is Python.', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
        if (r.status() === 200) {
    ...
    test('#206 memory isolation: no auth user has no memory', async ({ request }) => {
        test.skip(!siteIsUp);
        const r = await request.post('/api/chat', {
            data: { message: 'What do you remember about me?', avatar: 'kelion', language: 'en' }
        });
        if (r.status() === 503) { test.skip(); return; }
        expect(r.status()).toBeLessThan(500);
    });
