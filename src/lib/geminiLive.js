...
export function useGeminiLive({ audioRef, coords = null, onBalanceUpdate = null }) {
  ...
  const startCamera = useCallback(async (opts = {}) => {
    ...
    // succeeds on a lower rung, and anything that rejects explicit
    // resolutions still falls back to the permissive `video: true`.
    // When pickBestRearCameraDeviceId gives us a specific main lens
    // (non-ultrawide, non-tele) we put it at the top of the ladder.
