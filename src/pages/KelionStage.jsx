...
export default function KelionStage() {
  ...
  // Real client GPS (falls back to null â†’ server uses IP-geo instead).
  ...
  // a query (e.g. "aratÄƒ-mi harta" / "show me a map" without a place name).
  ...
  // loginâ†’topupâ†’usage funnel. Fetched alongside the raw rows.
  ...
  // never stuck with a side panel they cannot dismiss. Also closes the â‹¯
  ...
  // "Admin Â· âˆž" button and the tab bar both call â€” it closes whichever
  ...
  const refreshUsersList = useCallback(async (q = usersQuery, status = usersStatus) => {
    ...
    try {
      ...
      setUsersError(err.message || 'Nu am putut Ã®ncÄƒrca lista de useri')
    } finally {
  ...
  const banSelectedUser = useCallback(async (banned) => {
    ...
    let reason = null
    if (banned) {
      reason = window.prompt('Motiv suspendare (opÈ›ional):', '') || ''
    ...
    try {
      ...
      setSelectedResult({ ok: false, error: err.message || 'AcÈ›iunea a eÈ™uat' })
    } finally {
  ...
  const grantCreditsToSelected = useCallback(async () => {
    ...
    if (!Number.isFinite(minutes) || minutes === 0) {
      setSelectedResult({ ok: false, error: 'IntroduceÈ›i un numÄƒr diferit de 0' })
      return
    }
    const note = window.prompt('NotÄƒ (opÈ›ional):', '') || ''
    setSelectedBusy(true)
    setSelectedResult(null)
    try {
      ...
      setSelectedResult({
        ok: true,
        message: `${minutes > 0 ? 'AdÄƒugate' : 'Retrase'} ${Math.abs(Math.trunc(minutes))} minute Â· sold nou ${body.balance}`,
      })
      await Promise.all([loadUserDetail(selectedUserId), refreshUsersList()])
    } catch (err) {
      setSelectedResult({ ok: false, error: err.message || 'AcÈ›iunea a eÈ™uat' })
  ...
  const resetSelectedPassword = useCallback(async () => {
    if (!selectedUserId || selectedBusy) return
    if (!window.confirm('È˜terg parola + passkey-ul? Userul va trebui sÄƒ se relogheazÄƒ cu Google sau passkey nou.')) {
      return
    }
    setSelectedBusy(true)
    setSelectedResult(null)
    try {
      ...
      const body = await r.json().catch(() => ({}))
      if (!r.ok) throw new Error(body.error || `HTTP ${r.status}`)
      setSelectedResult({ ok: true, message: 'Parola + passkey È™terse. ContacteazÄƒ userul.' })
  ...
  const refreshDuplicateUsers = useCallback(async () => {
    ...
    try {
      ...
      setDupError(err && err.message ? err.message : 'Nu am putut Ã®ncÄƒrca conturile duplicate')
    } finally {
      setDupLoading(false)
    }
  }, [])
  const mergeDuplicateUsers = useCallback(async (sourceId, targetId, email) => {
    ...
    const confirmMsg =
      `Merge user ${sourceId} â†’ ${targetId} (${email})?\n\n` +
      'Toate conversaÈ›iile, creditele È™i istoricul sursei se vor muta pe È›intÄƒ.\n' +
      'Sursa va fi È™tearsÄƒ. AcÈ›iune ireversibilÄƒ.'
    ...
    try {
      ...
      setDupResult({
        ...
        targetId,
        email,
        error: err && err.message ? err.message : 'Merge eÈ™uat',
      })
    } finally {
      setDupBusyKey(null)
    }
  ...
  const triggerInstantPayout = useCallback(async () => {
    ...
    if (!window.confirm('Instant payout: transferÄƒ soldul disponibil pe cardul legat acum. Taxa Stripe ~1% + 0.25 EUR. ContinuÄƒm?')) {
      return
    }
    setPayoutBusy(true)
    setPayoutResult(null)
    try {
      const r = await fetch('/api/admin/payouts/instant', {
        ...
        credentials: 'include',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': getCsrfToken() },
        // Empty body â†’ Stripe pays out the full instant-available balance.
        body: JSON.stringify({}),
      })
      const body = await r.json().catch(() => ({}))
      if (!r.ok) {
  ...
  // â”€â”€â”€â”€â”€ Text-chat TTS (server-side ElevenLabs, male native voice) â”€â”€â”€â”€â”€
  ...
  // via `useAudioElementLipSync` (MediaElementSource â†’ analyzer), so the
  ...
  useEffect(() => {
    if (chatBusy) return
    if (muteModeRef.current) return   // mute mode â€” no TTS
    ...
    // types in â€” we forward it as a hint so short replies still route to a
    ...
    ;(async () => {
      ...
      try {
        ...
        audio.onplay = () => {
          ...
          // envelope a couple of frames later â€” if it's still 0 while
          ...
          const guard = () => {
            ...
            // 250 ms grace period for the analyser â€” fast enough that any
        ...
        // resort â€” the voice on Windows may be female.
        try {
  ...
  // â”€â”€ Mute mode â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  const MUTE_RE = /\b(nu (mai )?scoate (sun[Äƒe]t|audio|voc[Äƒe]|niciun sunet)|f(ii|Äƒ) (silenÈ›ios|silenÈ›|mut[Äƒe]?|tÄƒcut|liniÈ™tit)|fÄƒrÄƒ (sun[Äƒe]t|audio|voce)|opre[È™s]te (sunet|audio|vocea)|taci complet|silent( mode)?|mute|no (sound|audio|voice output)|nu vorbis?|nu mai vorbis?)\b/i
  const UNMUTE_RE = /\b(stop|reactivea[zz][Äƒa]|reactiveaz[Äƒa]|porneste (din nou|sunet|audio|vocea?)|unmute|activeaz[Äƒa] (sunet|audio|vocea?)|mai (vorbis?|scoate sunet)|vorbis?te( din nou)?)\b/i
  const TRANSLATOR_RE = /\b(asculta?[- ]?[È™s]i traduce|traduce [È™s]i(?: scrie|afis?ea[zz]Äƒ)?|interpret(ator|eazÄƒ)?|mod traduc|translator mode|audio ?to ?text|transcri(e|ere)|scrie ce (aud|se aude|spun))\b/i
  ...
  // the stage is mounted. Manual toggle via â‹¯ menu still works for users
  ...
  useEffect(() => {
    ...
    // Only stop on the signed-in â†’ signed-out transition, not on the
  ...
  // finishes (chatBusy trueâ†’false) and the last message is a finalised
  ...
  useEffect(() => {
    ...
    if (!prev || chatBusy) return // only on true â†’ false
    if (!authState.signedIn) return
    const msgs = chatMessagesRef.current
    const last = msgs[msgs.length - 1]
    if (!last || last.role !== 'assistant' || !last.content || !String(last.content).trim()) return
    // Convert {role, content} â†’ {role, text} for the extractor API.
    const snapshot = msgs
  ...
  const onStageClick = useCallback(() => {
    ...
    // First user gesture â†’ kick the geolocation permission prompt.
  ...
  // â”€â”€â”€â”€â”€ Wake-word "Kelion" â”€â”€â”€â”€â”€
    ...
    >
      ...
      >
        ...
        <input
          ...
          // Right-click â†’ Paste from the browser menu also fires
      ...
      {trialHud.applicable && trialHud.loaded && !authState.signedIn && (
        ...
        >
          <span aria-hidden style={{ fontSize: 13 }}>â±</span>
          {!trialHud.allowed ? (
      ...
      {/* Top-right action bar â€” Adrian: "panoul cu butoane e gÃ¢ndit
          greÈ™it". Simplified to: Credits/Admin pill + Sign in/out + â‹¯.
          Camera, screen, transcript, contact all moved into the â‹¯
      ...
      {menuOpen && (
        ...
        >
          {/* Camera / Screen share / Transcript â€” tools moved back into
              ...
              e gÃ¢ndit greÈ™it"). Camera also now auto-starts on speech/
          ...
          <MenuItem onClick={() => {
            ...
            {cameraStream ? 'ðŸ“¹ Turn camera off' : 'ðŸ“¹ Turn camera on'}
          </MenuItem>
          <MenuItem onClick={() => { screenStream ? stopScreen() : startScreen(); setMenuOpen(false) }}>
            {screenStream ? 'ðŸ–¥ï¸ Stop sharing screen' : 'ðŸ–¥ï¸ Share screen'}
          </MenuItem>
          <MenuItem onClick={() => { setTranscriptOpen((v) => !v); setMenuOpen(false) }}>
            {transcriptOpen ? 'ðŸ“ Hide transcript' : 'ðŸ“ Show transcript'}
          </MenuItem>
      ...
      {/* Contact moved to the top-bar as an icon (âœ‰ï¸) per Adrian's
      ...
      {/* Transcript drawer â€” opt-in, has X + backdrop + ESC to close.
          Previously the only way to close it was to re-open the â‹¯ menu
          ...
          >
            ...
            {!installed && !installPromptEvent && (
              <div style={{
                ...
                on iPhone, tap the Share button â†’ <em>Add to Home Screen</em>.
                On Android Chrome, tap â‹® â†’ <em>Install app</em>.
              </div>
            )}
          </div>
        </div>
      ...
      {creditsOpen && (
        ...
        >
            ...
            return (
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: 14, fontWeight: 600 }}>Revenue split ({pct}% â†’ AI)</div>
                  <span style={{
                ...
                >
                  Open GCP Billing dashboard â†’
                </a>
              </div>
            )
          })()}

          {/* â”€â”€â”€â”€â”€ Grant Credits â€” refund / comp / promo. Hits
          ...
          {/* â”€â”€â”€â”€â”€ Live Usage â€” Adrian: "analiza pe consum credite in timp
               ...
               happens, not post-mortem. â”€â”€â”€â”€â”€ */}
          <div style={{
            marginBottom: 16, padding: 14,
            borderRadius: 14,
            background: 'rgba(167, 139, 250, 0.05)',
                  ...
                  {suspects.length > 0 && (
                    <div style={{
                      ...
                      <div style={{ fontWeight: 700, marginBottom: 4 }}>
                        âš  Abnormal drain in last 5 min
                      </div>
          ...
          {!creditsLoading && autoTopupStatus && (() => {
            ...
            const lastRunLabel = (() => {
              ...
              if (e.status === 'ok') {
                return `Ultima reÃ®ncÄƒrcare: ${id} Â· ${e.amountEur} ${String(e.currency || 'eur').toUpperCase()} Â· ${when}`
              }
              return `Ultima Ã®ncercare: ${id} Â· eÈ™uatÄƒ (${e.error || 'eroare necunoscutÄƒ'}) Â· ${when}`
            })()
            return (
              <div style={{
                ...
                <div style={{ fontWeight: 600, marginBottom: 4 }}>
                  {armed
                    ? `Auto-topup armat â€” sub ${thresholdPct}% cardul tÄƒu Stripe e taxat cu ${s.amountEur} ${String(s.currency || 'eur').toUpperCase()}.`
                    : 'Auto-topup inactiv â€” leagÄƒ un card salvat Ã®n Stripe ca sÄƒ activezi.'}
                </div>
                <div style={{ fontSize: 12, opacity: 0.85 }}>
                  {armed
                    ? `VerificÄƒm la fiecare deschidere a panoului. Cooldown ${s.cooldownHours || 24}h ca sÄƒ nu se Ã®ncarce de douÄƒ ori. Primim email de confirmare sau eroare.`
                    : 'SeteazÄƒ OWNER_STRIPE_CUSTOMER_ID + OWNER_STRIPE_PAYMENT_METHOD_ID Ã®n Railway, apoi refresh. Cardul Ã®l salvezi o datÄƒ Ã®n Stripe.'}
                </div>
                {lastRunLabel && (
                ...
                {!armed && (
                  ...
                  >Deschide Stripe â€” Customers â†’</a>
                )}
              </div>
            )
          })()}
          ...
          {!creditsLoading && creditsCards.map((c) => {
            ...
            // "credit aproape terminat" / "cheie lipsÄƒ" instead of
            ...
            return (
              ...
              >
                ...
                <div style={{
                  ...
                  Tap to open {c.kind === 'revenue' ? 'dashboard' : 'top-up'} â†’
                </div>
              </a>
            )
          })}
              ...
              <>
                {hiddenBots > 0 && (
                  <div style={{
                    ...
                    {hiddenBots} hit{hiddenBots !== 1 ? '-uri' : ''} de boÈ›i / scanere ascunse din listÄƒ.
                  </div>
                )}
                <div style={{
                  borderRadius: 12,
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{ opacity: 0.65, fontSize: 11 }}>
                          {browser} Â· {os}
                          {ref && <span style={{ opacity: 0.7 }}> Â· â† {ref}</span>}
                        </div>
                      </div>
                    )
                  })}
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
            <input
              ...
              placeholder="CautÄƒ dupÄƒ email, nume sau IDâ€¦"
              style={{
            ...
            >
              ...
              <option value="banned">SuspendaÈ›i</option>
            ...
            >
              {usersLoading ? 'Se Ã®ncarcÄƒâ€¦' : 'ReÃ®ncarcÄƒ'}
            </button>
          </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              >
                {dupLoading ? 'Se verificÄƒâ€¦' : 'ReÃ®ncarcÄƒ'}
              </button>
            </div>
            {dupError && (
              <div style={{
            ...
            {dupResult && (
              <div style={{
                ...
                {dupResult.ok ? (
                  <>
                    Merge reuÈ™it: user {dupResult.sourceId} â†’ {dupResult.targetId}
                  ...
                  <>Merge eÈ™uat ({dupResult.sourceId} â†’ {dupResult.targetId}): {dupResult.error}</>
                )}
              </div>
            )}
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: 11, opacity: 0.6, marginBottom: 8 }}>
                    PÄƒstrÄƒm contul cel mai vechi (primul din listÄƒ) ca È›intÄƒ.
                    Butonul "Merge â†’ {canonical ? `#${canonical.id}` : 'â€¦'}"
                    mutÄƒ totul de pe peer pe el È™i È™terge peer-ul.
                  </div>
                  {(g.users || []).map((u, idx) => {
                    ...
                    return (
                      ...
                      >
                          ...
                          <div style={{ fontWeight: 600, marginBottom: 2 }}>
                            ...
                            {isCanonical && (
                              <span style={{
                                ...
                                â† È›intÄƒ (se pÄƒstreazÄƒ)
                              </span>
                            )}
                          </div>
                          <div style={{ opacity: 0.6, fontSize: 11 }}>
                            {u.google_id ? 'Google Â· ' : ''}
                            {u.password_hash ? 'parolÄƒ Â· ' : ''}
                        ...
                        {!isCanonical && canonical && (
                          ...
                          >
                            {busy ? 'Mergeâ€¦' : `Merge â†’ #${canonical.id}`}
                          </button>
                        )}
                      </div>
                    )
          ...
          {usersData && (
            <div style={{ fontSize: 12, opacity: 0.65, marginBottom: 8 }}>
              ...
              {usersData.query ? ` Â· filtrat dupÄƒ â€ž${usersData.query}"` : ''}
            </div>
          )}
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                    <span style={{ fontWeight: 600 }}>{u.email || '(fÄƒrÄƒ email)'}</span>
          ...
          {/* User detail sub-drawer â€” overlays the list when a row is
              clicked. Close via "â† ÃŽnapoi la listÄƒ" or by picking
          ...
          {selectedUserId && (
            <div style={{
              ...
              {!selectedUser && (
                <div style={{ opacity: 0.6, fontSize: 13 }}>Se Ã®ncarcÄƒ detaliileâ€¦</div>
              )}
                ...
                <>
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
                    ...
                    {selectedUser.banned ? (
                      ...
                      >SuspendÄƒ contul</button>
                    )}
                    <button
                      onClick={resetSelectedPassword}
                      disabled={selectedBusy}
                      style={actionBtnStyle(selectedBusy)}
                    >ReseteazÄƒ parola</button>
                  </div>

                  {/* History panel */}
                  <div style={{ fontSize: 11, opacity: 0.6, marginBottom: 6, letterSpacing: '0.1em' }}>
                    ISTORIC Â· ULTIMELE {selectedHistory?.rows?.length || 0} TRANZACÈšII
                  </div>
        ...
        >
          ...
          <div style={{
            ...
            <div style={{ opacity: 0.82 }}>
              Stripe varsÄƒ automat soldul Ã®n contul/ cardul pe care l-ai
              conectat ca "external account". Nu trebuie sÄƒ iniÈ›iezi tu
              nimic â€” odatÄƒ configurat, fiecare top-up al unui user trece
              prin: Stripe Checkout â†’ Stripe balance â†’ payout automat (zilnic
              sau sÄƒptÄƒmÃ¢nal, dupÄƒ setarea ta). JumÄƒtate din fiecare top-up
              e deja rezervatÄƒ intern pentru costurile AI (OpenAI, Groq,
              ElevenLabs), cealaltÄƒ jumÄƒtate e profitul net.
            </div>
          </div>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>SeteazÄƒ destinaÈ›ia payout-urilor</span>
              <span style={{ fontSize: 12, opacity: 0.6 }}>Stripe â†—</span>
            </div>
            <div style={{ fontSize: 11, opacity: 0.65, marginTop: 4 }}>
              Adaugi un IBAN sau un card de debit o singurÄƒ datÄƒ. Recomandat:
              Visa/Mastercard Debit (Revolut, Wise, Starling) pentru plÄƒÈ›i
              instant Ã®n 30 min.
            </div>
          </a>
          ...
          >
            ...
            <div style={{ fontSize: 11, opacity: 0.65, marginTop: 4 }}>
              Fiecare platÄƒ cÄƒtre banca/cardul tÄƒu, cu data È™i suma.
            </div>
          </a>
...
// look with the â‹¯ overflow button â€” an accent ring appears when `active`
