...
export default function KelionStage() {
  ...
  return (
    ...
    >
      ...
      {transcriptOpen && (() => {
        // Transcript drawer state — kept inline so it resets when the
        // drawer closes (no stale results leaking across sessions).
        const [tsTab, setTsTab] = React.useState('live')
        const [tsQuery, setTsQuery] = React.useState('')
        const [tsDateFrom, setTsDateFrom] = React.useState('')
        const [tsDateTo, setTsDateTo] = React.useState('')
        const [tsTimeFrom, setTsTimeFrom] = React.useState('')
        const [tsTimeTo, setTsTimeTo] = React.useState('')
        const [tsRole, setTsRole] = React.useState('')
        const [tsResults, setTsResults] = React.useState([])
        const [tsTotal, setTsTotal] = React.useState(0)
        const [tsOffset, setTsOffset] = React.useState(0)
        const [tsLoading, setTsLoading] = React.useState(false)
        const [tsError, setTsError] = React.useState(null)
        const [tsFiltersOpen, setTsFiltersOpen] = React.useState(false)
        const tsLimit = 30
        const tsDebounceRef = React.useRef(null)

        const doSearch = React.useCallback(async (newOffset = 0) => {
          if (!authState.signedIn) {
        ...
        }, [tsQuery, tsDateFrom, tsDateTo, tsTimeFrom, tsTimeTo, tsRole, authState.signedIn])
        ...
        }, [tsQuery, tsDateFrom, tsDateTo, tsTimeFrom, tsTimeTo, tsRole, tsTab])
