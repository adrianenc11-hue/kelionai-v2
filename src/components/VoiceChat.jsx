    ...
    try {
      ...
      if (r.ok) {
        ...
        if (!r.ok) {
          ...
          throw new Error(err.error || 'Could not obtain token')
        }
        tokenData = await r.json()
      }
      ...
      if (tokenData.trial) {
        ...
        if (remaining <= 0) { setStatus('error'); setAiText('Your trial has expired. Create an account to continue.'); return }
        setTimeLeft(remaining)
        timerRef.current = setInterval(() => {
          setTimeLeft(prev => {
            if (prev <= 1) { clearInterval(timerRef.current); disconnect(); setAiText('Your trial has expired. Create an account!'); return 0 }
            return prev - 1
          })
        }, 1000)
      }
      ...
      <div style={{ width:400,display:'flex',flexDirection:'column', background:'rgba(0,0,0,0.35)',borderLeft:'1px solid rgba(255,255,255,0.07)' }}>
        ...
        <div style={{ padding:'16px 20px',borderTop:'1px solid rgba(255,255,255,0.07)',flexShrink:0,display:'flex',flexDirection:'column',gap:10 }}>
          {!live ? (
            ...
            }}>{status==='connecting'?'⏳ Connecting…':status==='error'?'🔄 Try again':'🎤 Start Chat'}</button>
          ) : (
