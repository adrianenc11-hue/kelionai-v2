...
function main() {
  ...
  if (check.status !== 0) {
    log(
      "CHECK",
      "⚠️  Verificările au eșuat. Rezolvă problemele de mai sus înainte de deploy complet.",
    );
    process.exit(1);
  }
  ...
  if (dep.status !== 0) {
    log(
      "RAILWAY",
      "⚠️  Deploy-ul backend a eșuat. Verifică log-urile Railway / scripts/deploy.sh.",
    );
    process.exit(1);
  }
  ...
  log(
    "NETLIFY",
    "Încerc să pornesc workflow-ul GitHub 'Manual Deploy' pentru Netlify (environment=production)...",
  );
  ...
  const wf = spawnSync(
    "gh",
    ["workflow", "run", "deploy-only.yml", "-f", "environment=production"],
  ...
  if (wf.status !== 0) {
    ...
    log(
      "NETLIFY",
      "✅ Workflow 'Manual Deploy' pornit. Urmărește statusul în GitHub → Actions.",
    );
  }

  log(
    "DONE",
    "Deploy complet lansat. Backend pe Railway + frontend pe Netlify (kelionai.app) vor fi actualizate după finalizarea pipeline-urilor.",
  );
}
