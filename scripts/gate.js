...
(async () => {
  ...
  results.push(
    runGate("Prettier (format check)", 'npx prettier --check "server/**/*.js"'),
  );
  ...
  if (smokeUrl) {
    results.push(
      runGate("Smoke Probe", `npm run smoke --silent`, { timeout: 60_000 }),
    );
    if (results.at(-1).status === "fail") failed = true;
  } else {
    log("\n════ GATE: Smoke Probe ════");
  ...
  if (smokeUrl) {
    ...
    log("\n════ GATE: E2E (Playwright) ════");
    log(
      "⏭️  SKIPPED — no BASE_URL set (requires Railway preview URL per Truth Guard rules)",
    );
    results.push({
      name: "E2E (Playwright)",
      status: "skipped",
  ...
  const report = {
    ...
    limitations: [
      ...(smokeUrl
        ? []
        : ["Smoke + E2E skipped — no BASE_URL (local run only)"]),
  ...
  for (const r of results) {
    ...
    log(
      `  ${icon} ${r.name}: ${r.status.toUpperCase()}${r.duration ? ` (${r.duration}ms)` : ""}`,
    );
  }
