...
SCHEMA_FILE=""
if [ -f "$PROJECT_DIR/server/schema-full.pgsql" ]; then
    SCHEMA_FILE="$PROJECT_DIR/server/schema-full.pgsql"
    ok "Folosesc: server/schema-full.pgsql"
elif [ -f "$PROJECT_DIR/server/schema.sql" ]; then
    SCHEMA_FILE="$PROJECT_DIR/server/schema.sql"
    ok "Folosesc: server/schema.sql"
else
    err "Nu am găsit fișierul schema SQL în server/schema.sql sau server/schema-full.pgsql"
    exit 1
fi
