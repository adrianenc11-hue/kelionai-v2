...
(async () => {
  ...
  for (const r of routes) {
    try {
      ...
      console.log(
        `  ${mark}  ${r.path.padEnd(22)} ${status}${extra}  [${r.desc}]`,
      );
    } catch (e) {
      failed++;
      console.log(
        `  ❌ FAIL  ${r.path.padEnd(22)} ERROR: ${e.message}  [${r.desc}]`,
      );
    }
  }
  ...
  for (const c of runtimeChecks) {
    try {
      ...
      if (hasRo) roFound++;
      console.log(
        `  ${hasRo ? "❌ ROMANIAN FOUND" : "✅ ENGLISH ONLY"}  ${c.path.padEnd(22)}  [${c.desc}]`,
      );
    } catch (e) {
  ...
  for (const p of directRoutes) {
    ...
    console.log(
      `  ${ok ? "✅ PASS" : "❌ FAIL"}  ${p.padEnd(15)} access=${r1.status}  refresh=${r2.status}`,
    );
  }
  ...
  console.log(
    `  Romanian:  ${roFound === 0 ? "0 found ✅" : roFound + " found ❌"}`,
  );
