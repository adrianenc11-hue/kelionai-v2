...
function checkRailway() {
  ...
  if (ver.status !== 0) {
    log(
      "RAILWAY",
      "⚠️  Railway CLI nu este instalat sau nu este în PATH. Rulează: npm i -g @railway/cli",
    );
    return;
  }
  ...
  if (status.status !== 0) {
    log(
      "RAILWAY",
      "⚠️  Niciun proiect Railway link-uit. Rulează: railway link (din acest director).",
    );
  } else {
  ...
  if (missing.length === 0) {
    log(
      "RAILWAY",
      "✅ Cheile critice pentru Railway există în .env (GOOGLE_AI_KEY + SUPABASE_*)",
    );
  } else {
    log(
      "RAILWAY",
      `⚠️  Lipsesc sau sunt placeholder în .env: ${missing.join(", ")}. Rulează: npm run railway:setup`,
    );
  }
}
...
function checkNetlifySecrets() {
  ...
  if (auth.status !== 0) {
    log(
      "NETLIFY",
      "⚠️  gh nu este autentificat. Rulează: gh auth login (apoi rulează din nou check:deploy).",
    );
    return;
  }
  ...
  if (secrets.status !== 0) {
    log(
      "NETLIFY",
      "⚠️  Nu pot lista secretele GitHub. Verifică permisiunile pentru gh.",
    );
    return;
  }
  ...
  if (hasToken && hasSite) {
    log(
      "NETLIFY",
      "✅ NETLIFY_AUTH_TOKEN și NETLIFY_SITE_ID sunt setate ca GitHub Secrets.",
    );
  } else {
    const missing = [];
    ...
    log(
      "NETLIFY",
      `⚠️  Lipsesc secretele: ${missing.join(
        ", ",
      )}. Rulează: bash scripts/setup-secrets.sh`,
    );
  }
}
...
function main() {
  ...
  log(
    "CHECK",
    "Verificări terminate. Citește mesajele de mai sus pentru pașii următori (dacă există ⚠️).",
  );
}
